...
type jsonOutputWebAPIAuthAssociation struct {
	...
	RetryInterval string `json:"retrytime"`
...
func WebServer_Auth_API_Service_Invalid_Access_Response(w http.ResponseWriter, Code string, Message string) {
...
func ProvisioningUploadDBSetTransaction(Database *sql.DB, keyid int64, userid int64, deviceid int, InputData *jsonInputWebAPIAuthProvisioningPack, OutputData *jsonOutputWebAPIAuthProvisioningPack, proviReq *ProvisionProtocol) int {
	...
	defer tx.Rollback()
	QueryString = "INSERT INTO MCSE_Info (device_id, user_id, key_id, status, max_connections, receive_buffer_size, send_buffer_size, " +
		...
		"kms_addr, kms_port, node_ip, site_id) " +
		"VALUES (?, ?, ?, ?, ?, ?, ?," +
		"?, ?, ?, " +
		"?, ?, ?, ?, ?, ?, " +
		"?, ?, ?, ?, ?, " +
	...
	logFile := filepath.Base(proviReq.Body.Data.Logfile_Path)
	logDir := strings.TrimRight(proviReq.Body.Data.Logfile_Path, logFile)

	errFile := filepath.Base(proviReq.Body.Data.Err_Logfile_Path)
	errDir := strings.TrimRight(proviReq.Body.Data.Err_Logfile_Path, errFile)

	_, err = stmt.Exec(deviceid, userid, keyid, "000", max_connections, receive_buffer_size, send_buffer_size,
		time_connect, time_client, time_server,
		limit_size_log, max_size_log_file, logDir, logFile, errDir, errFile,
		1, statistics_cycle, proviReq.Body.Data.Statistic_Server_Ip, statistics_port, statistics_data_cycle,
		0, mcs_size, encrypt, 0,
		proviReq.Body.Data.KMS_Address, kms_port, InputData.IP, 1)
...
func ProvisioningUploadDBUpdateTransaction(Database *sql.DB, deviceid int, InputData *jsonInputWebAPIAuthProvisioningPack, OutputData *jsonOutputWebAPIAuthProvisioningPack, proviReq *ProvisionProtocol) int {
	...
	QueryString = "UPDATE MCSE_Info SET " +
		"max_connections = ?, receive_buffer_size = ?, send_buffer_size = ?, " +
		"time_connect = ?, time_client = ?, time_server = ?, " +
		"limit_size_log = ?, max_size_log_file = ?, log_file_path_dir = ?, log_file_path_name = ?, error_log_file_path_dir = ?, error_log_file_path_name = ?, " +
		"statistics_yn = ?, statistics_cycle = ?, statistics_ip = ?, statistics_port = ?, statistics_data_cycle = ?, " +
		"bridge_yn = ?, mcs_size = ?, encrypt_mode = ?, change_ip_mode = ?, " +
		"kms_addr = ?, kms_port = ?,node_ip = ? " +
		"WHERE device_id = ? "
	log.Println("Query:", QueryString)
	...
	_, err = stmt.Exec(max_connections, receive_buffer_size, send_buffer_size,
		time_connect, time_client, time_server,
		limit_size_log, max_size_log_file, logDir, logFile, errDir, errFile,
		1, statistics_cycle, proviReq.Body.Data.Statistic_Server_Ip, statistics_port, statistics_data_cycle,
		0, mcs_size, encrypt, 0,
		proviReq.Body.Data.KMS_Address, kms_port, InputData.IP,
		deviceid)
	if err != nil {
		stmt.Close()
		log.Println("Exec Fail!:", err)
...
func SelectDBConfigData(db *sql.DB, deviceid int) (*SettingsInformation, string, error) {
	...
	query := "Select A.user_key , ISNULL(B.password, ''), B.max_connections , B.receive_buffer_size, B.send_buffer_size, B.time_connect, B.time_client, B.time_server\n" +
	...
	for rows.Next() {
		...
		if settingData.Encrypt_Mode == "001" {
			settingData.Encrypt_Mode = "None"
		} else if settingData.Encrypt_Mode == "002" {
			settingData.Encrypt_Mode = "AES_128"
		} else if settingData.Encrypt_Mode == "003" {
			settingData.Encrypt_Mode = "AES_256"
		} else if settingData.Encrypt_Mode == "004" {
			settingData.Encrypt_Mode = "RC4"
		} else {
			settingData.Encrypt_Mode = "None"
		}

		if settingData.Password != "" {
			settingData.Password = EncryptGoWebPassword(settingData.Password)
		}
	...
	if errDir != "/" {
		errDir = strings.TrimRight(errDir, "/")
	}
...
func WebServer_Web_Auth_API_Provisioning_Proc(w http.ResponseWriter, req *http.Request) {
	...
	if InputData.AuthKey == "" && InputData.AuthToken == "" {
		...
		} else if DBNodeIDStatus == "004" {
			if InputData.DeviceID == "0" {
				//--[Query: Update Resource UserKey Status]-----{
				QueryString = "UPDATE mcs.Node_ID SET status = '001' WHERE device_id = %d and status = '004' "
				...
				if err != nil {
					log.Println("db processing error (failed to update start state of node_id ready status)")
					ResponseMessage := "db processing error (failed to update start state of node_id ready status)"
					WebServer_Auth_API_Provisioning_Response(w, InputData.Version, InputData.Method, InputData.SessionType, "response", "630", ResponseMessage, "", "", "", "", "")
					return
				}

				log.Printf("status update [userkey:%s, nodeid:%s] [status 004 -> 001]", InputData.UserKey, InputData.DeviceID)
			} else {
				log.Println("not supported case (device id is zero and node id status is non ready)")
				ResponseMessage := "not supported case (device id is zero and node id status is non ready)"
				WebServer_Auth_API_Provisioning_Response(w, InputData.Version, InputData.Method, InputData.SessionType, "response", "630", ResponseMessage, "", "", "", "", "")
				return
			}
		...
		if proviReq.Header.Seperator == "up" {
			...
			if InputData.IP == "" {
				...
				err = UpdateProvisioningNodeip(Database, DBDeviceID, InputData.IP)
				if err != nil {
			...
			*/
			log.Println("proviReq.Header:", proviReq.Header)

			newpkgversion, updatefilename := GetNewPkgVersion(Database, InputData.UserKey, proviReq.Header.OSVersion, proviReq.Header.CurPkgVersion)
			...
			proviRes.Header.NewPkgVersion = newpkgversion
			proviRes.Header.UpdateFileName = updatefilename
...
func GetNewPkgVersion(Database *sql.DB, UserKey string, OSVersion string, CurPkgVersion string) (string, string) {
	var QueryStr string
	var Rows *sql.Rows
	var err error
	var UpdateSig bool
	var NewPkgVersion string
	var UpdateFileName string

	QueryStr = "select auto_update_yn from User_Key where user_key = '%s'"
	QueryStr = fmt.Sprintf(QueryStr, UserKey)

	Rows, err = msdb_lib.Query_DB(Database, QueryStr)
	if err != nil {
		log.Println("Query err", err)
	}
	for Rows.Next() {
		err = Rows.Scan(&UpdateSig)
		if err != nil {
			break
		}
	}
	...
	if UpdateSig == true {
		...
		QueryStr = "select top (1) version,update_file from mcs.MCSE_Package where LOWER(os) = LOWER('%s') and update_yn = 1 and GETDATE() > booking_date order by package_id DESC;"
		QueryStr = fmt.Sprintf(QueryStr, OSVersion)
		Rows, err = msdb_lib.Query_DB(Database, QueryStr)
		if err != nil {
			log.Println("Query err", err)
		}
		for Rows.Next() {
			err = Rows.Scan(&NewPkgVersion, &UpdateFileName)
			if err != nil {
				Rows.Close()
				break
			}
		...
		if NewPkgVersion == "" {
			log.Println("No New Package version")
			return "", ""
		} else {

			NewPkgVer := ComparePkgVersion(CurPkgVersion, NewPkgVersion)
			if NewPkgVersion == "" {
				log.Println("No New Package version")
				return "", ""
			} else {
				return NewPkgVer, UpdateFileName
			}
		}
	} else {
		log.Println("No New Package version")
...
func ComparePkgVersion(curpkgversion string, newpkgversion string) string {

	CurPkgVer, err := version.NewVersion(curpkgversion)
	if err != nil {
		fmt.Println("[ERR] v1 err:", err)
	}

	NewPkgVer, err := version.NewVersion(newpkgversion)
	if err != nil {
		fmt.Println("[ERR] v1 err:", err)
	}

	if CurPkgVer.LessThan(NewPkgVer) {
		log.Println("[LOG] have to update new package")
...
func WebServer_Auth_API_Association_Response(w http.ResponseWriter, Version string, Method string, SessionType string, Seperator string, MessageType string, MessageSeq string, Code string, Message string, AuthKey string, Expiretime string, Retrytime string, Event string) {
	...
	OutputData.AuthKey = AuthKey
	OutputData.Expiretime = Expiretime
	OutputData.RetryInterval = Retrytime
...
func WebServer_Auth_API_Association_Proc(w http.ResponseWriter, req *http.Request) {
	...
	var RetStoredTrafficResultTraffic uint32
	var RetStoredTrafficResultCode string
	...
	// kwgwak77@innogs.com { #13299
	AuthExpiretimeInterval = (60)
	AuthRetryInterval = (60 * 60)
	// kwgwak77@innogs.com } #13299
	...
	if InputData.AuthToken == "" {
		...
		if GenerateAuthToken != "" {
			...
			WebServer_Auth_API_Association_Response(w, InputData.Version, InputData.Method, InputData.SessionType, InputData.Seperator, "response", InputData.MessageSeq, "200", ResponseMessage, GenerateAuthKey, strconv.Itoa(AuthExpiretimeInterval), strconv.Itoa(AuthRetryInterval), "")

			log.Printf("web api response [userkeyid:%d, deviceid:%d] [code:%s, msg:%s, (expiretime sec:%d, retrytime sec:%d, authkey:%s, authtoken:%s)]", DBUserKeyID, DBDeviceID, "200", ResponseMessage, AuthExpiretimeInterval, AuthRetryInterval, GenerateAuthKey, GenerateAuthToken)
			return
		} else {
      ...
      // calculate giga byte (GB -> B) (1GB : 1073741824 Byte = 1024 * 1024 * 1024 Byte) //
      TrafficLimitInboundBytes = (TrafficLimitInboundGiGaBytes * 1073741824)

      ErrorFlag = 0
      //-----------------------------------------------{
      QueryString = "SELECT ISNULL(sum(tmp.inbound), 0) AS sum_up_traffic_byte " +
      ...
      if QueryTupleCount > 1 {
        ...
        if TrafficLimitInboundBytes < (TrafficSumInboundBytes) {
          ...
          // DB stored traffic addtion procedure ----------------------------------------------{
          QueryString = "EXEC mcs.get_stored_traffic %d, %d, %d "
          QueryString = fmt.Sprintf(QueryString, DBDeviceID, 2, 0)
          log.Println("Stored Traffic Auto Addtion Procedure Query -> [", QueryString, "]")
          // DB stored traffic addtion procedure ----------------------------------------------}
          ResultSetRows, err = msdb_lib.Query_DB(Database, QueryString)
          if err != nil {
            log.Println("db query error (failed to add stored traffic procedure)")
            ResponseMessage = "db query error (failed to add stored traffic procedure)"
            WebServer_Auth_API_Association_Response(w, InputData.Version, InputData.Method, InputData.SessionType, InputData.Seperator, "response", InputData.MessageSeq, "630", ResponseMessage, "", "", "", "")
            return
          }
          ...
          QueryTupleCount = 0
          for ResultSetRows.Next() {
            err := ResultSetRows.Scan(&RetStoredTrafficResultTraffic, &RetStoredTrafficResultCode)
            if err != nil {
              ErrorFlag = 1
              ResultSetRows.Close()
              log.Println("db query error (not founded sum (inbound) of mcs.Server_Statistics_Info)) (errmsg:", err, ")")
          ...
          if QueryTupleCount != 1 {
            log.Println("db query error (not founded return value of add stored traffic procedure)")
            ResponseMessage = "db query error (not founded return value of add stored traffic procedure))"
            WebServer_Auth_API_Association_Response(w, InputData.Version, InputData.Method, InputData.SessionType, InputData.Seperator, "response", InputData.MessageSeq, "630", ResponseMessage, "", "", "", "")
            return
          }

          if RetStoredTrafficResultCode == "000" {
            log.Printf("success processing stored traffic procedure [userkeyid:%d, deviceid:%d] [code:%s, msg:%s] (current limit byte:%d, current sum bytes:%d)", DBUserKeyID, DBDeviceID, RetStoredTrafficResultCode, "normal traffic operation", TrafficLimitInboundBytes, TrafficSumInboundBytes)
          } else if RetStoredTrafficResultCode == "001" {
            log.Printf("success processing stored traffic procedure [userkeyid:%d, deviceid:%d] [code:%s, addition:%d, msg:%s] (current limit byte:%d, current sum bytes:%d)", DBUserKeyID, DBDeviceID, RetStoredTrafficResultCode, RetStoredTrafficResultTraffic, "stored traffic addition", TrafficLimitInboundBytes, TrafficSumInboundBytes)
          } else if RetStoredTrafficResultCode == "101" || RetStoredTrafficResultCode == "102" || RetStoredTrafficResultCode == "103" || RetStoredTrafficResultCode == "104" {
            //-----------------------------------------------{
            QueryString = "DELETE FROM mcs.CWS_Auth " +
              "WHERE key_id = %d and device_id = %d and method = '%s' and session_type = '%s' "
            QueryString = fmt.Sprintf(QueryString, DBUserKeyID, DBDeviceID, InputData.Method, InputData.SessionType)
            ...
            if RetStoredTrafficResultCode == "101" || RetStoredTrafficResultCode == "102" {
              ResponseMessage = "db query error (not founded tuple of stored traffic procedure)"
              WebServer_Auth_API_Association_Response(w, InputData.Version, InputData.Method, InputData.SessionType, InputData.Seperator, "response", InputData.MessageSeq, "630", ResponseMessage, "", "", "", "")
              log.Printf("web api response [userkeyid:%d, deviceid:%d] [code:%s, msg:%s]", DBUserKeyID, DBDeviceID, "630", ResponseMessage)
              return
            } else if RetStoredTrafficResultCode == "103" {
              ResponseMessage = "product service restrictions (measured rate system : GB of stored traffic procedure)"
              ...
              return
            } else if RetStoredTrafficResultCode == "104" {
              ResponseMessage = "db query error (invalid type of stored traffic procedure)"
              WebServer_Auth_API_Association_Response(w, InputData.Version, InputData.Method, InputData.SessionType, InputData.Seperator, "response", InputData.MessageSeq, "653", ResponseMessage, "", "", "", "")
              log.Printf("web api response [userkeyid:%d, deviceid:%d] [code:%s, msg:%s]", DBUserKeyID, DBDeviceID, "630", ResponseMessage)
              return
            }
          } else {
            ResponseMessage = "db query error (not supported return of stored traffic procedure)"
            WebServer_Auth_API_Association_Response(w, InputData.Version, InputData.Method, InputData.SessionType, InputData.Seperator, "response", InputData.MessageSeq, "653", ResponseMessage, "", "", "", "")
            log.Printf("web api response [userkeyid:%d, deviceid:%d] [code:%s, msg:%s]", DBUserKeyID, DBDeviceID, "630", ResponseMessage)
            return
          }
		...
		ResponseMessage = "auth succ (expiretime udate)"
		WebServer_Auth_API_Association_Response(w, InputData.Version, InputData.Method, InputData.SessionType, InputData.Seperator, "response", InputData.MessageSeq, "200", ResponseMessage, "", strconv.Itoa(AuthExpiretimeInterval), strconv.Itoa(AuthRetryInterval), "")
		log.Printf("web api response [userkeyid:%d, deviceid:%d] [code:%s, msg:%s (expiretime sec:%d, retrytime sec:%d, authtoken:%s)]", DBUserKeyID, DBDeviceID, "200", ResponseMessage, AuthExpiretimeInterval, AuthRetryInterval, DBAuthToken)
		return
	}
}
...
func RunWebContainer(ServicePort string) {
	...
	WebServerMux.HandleFunc("/", func(w http.ResponseWriter, req *http.Request) {
		...
		//WebServer_Redirect(w, req, "/login/")
		WebServer_Auth_API_Service_Invalid_Access_Response(w, "403", "Forbidden")
	})
...
func UpdateDBNodeStatus(db *sql.DB) error {
	query := "UPDATE MCSE_Info\n" +
		"SET status = '002'\n" +
		"WHERE DATEADD(ss,90,provisioning_time) < GETDATE()  AND  status != '000' ;"
...
func InsertStatisticsDB(Statistics []StatisticInformation, DBDeviceID int) error {
	...
	if Statistics[0].Type == "001" {
		QueryString = "INSERT INTO mcs.Server_Statistics_Info (device_id,key_id,node_id,server_ip,server_port,mcse_ip,\n" +
			"mcse_port,proxy_ip,client_ip,inbound,outbound,reg_date,site_id)\n" +
			"VALUES %s"
		...
		QueryString = "INSERT INTO mcs.Client_Statistics_Info (device_id,key_id,node_id,mcse_ip,mcse_port,proxy_ip,\n" +
			"proxy_port,client_ip,inbound,outbound,reg_date,site_id)\n" +
			"VALUES %s"
...
func main() {
	...
	if ProcessType == "bg" {
		context := &daemon.Context{
			PidFileName: "innogs_svc_gowas.pid",
			PidFilePerm: 0644,
			LogFileName: ProcessLogFileName,
			LogFilePerm: 0640,
			WorkDir:     "./",
			Umask:       027,
			Args:        []string{"./innogs_svc_gowas", "-l", ListenerPort, "-p", ProcessType},
		}
