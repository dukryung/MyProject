...
func WebServer_Server_Statistics(w http.ResponseWriter, req *http.Request, Database *sql.DB) {
	...
	if ControlServerFlag == 0 {
		...
		TempStr = fmt.Sprintf("<th><a href=\"/statistics/server/?page_num=1&sort=%d%s\">SML Server IP %s</a></th>", SortNodeIP, Params, SortNodeIPDir)
...
func WebServer_Web_Auth_API_Session_AuthToken(w http.ResponseWriter, req *http.Request) {
	...
	log.Println(">>> Input Data : [method:" + InputData.Method + ", sessiontype:" + InputData.SessionType + ", userkey:" + InputData.UserKey + ", authkey:" + InputData.AuthKey + ", userkeyid:" + InputData.UserKeyID + ", deviceid:" + InputData.DeviceID + "]")
	...
	if InputData.UserKeyID == "" {
		InputData.UserKeyID = "0"
	}
	if InputData.DeviceID == "" {
		InputData.DeviceID = "0"
	}
...
func StatisticsUploadLocalProcess(lwsDatabase *sql.DB, trafficDatabase *sql.DB) {
	...
	var Statistics []StatisticInformation
	//var SendStatistics []StatisticInformation
	var Limitnum, ServOffsetnum, ClntOffsetnum int64
	var ServOffsetnumstr, ClntOffsetnumstr string
	var indexrowscnt int
	var stmt *sql.Stmt
	var tx *sql.Tx
	...
	for {
		...
		/*QueryStr = "select A.ID, A.Time, A.Bridge_ID_TEXT , A.Proxy_IP_INT, A.Proxy_IP_TEXT, A.Node_IP_INT,\n" +
			...
			"from SERVER_STATISTICS_COMMON A, SERVER_STATISTICS_DATA B where A.ID = B.ID OFFSET %d"
		QueryStr = fmt.Sprintf(QueryStr, ServOffsetnum)
		...
		StatRows.Close()
			...
			for idx := range Statistics {

				SendStatistics = append(SendStatistics, Statistics[idx])

				if len(SendStatistics) == 100 {
					...
					if err := StatisticsUploadRequest(lwsDatabase, url, userstlbData, authData, authkey, authtoken, SendStatistics); err != nil {
						log.Println(err)
						SendStatistics = nil
						Statistics = nil
						StatRows.Close()
					...
					ServOffsetnum += len(SendStatistics)
					SendStatistics = nil
				} else if len(SendStatistics) != 100 && len(SendStatistics) > 0 {
					authkey, authtoken, err := StatisticsAuthRequest(lwsDatabase, url, authData)
					if err != nil {
						log.Println(err)
						SendStatistics = nil
		...
		*/
		...
		QueryStr = "select Count(*) from StatIdxTbl"
		...
		for StatRows.Next() {
			err = StatRows.Scan(&indexrowscnt)
			if err != nil {
				log.Println("Scan error", err)
				StatRows.Close()
				break
		...
		QueryStr = "select ifnull(ServIdx , '%s'), ifnull(ClntIdx , '%s') from StatIdxTbl"
		QueryStr = fmt.Sprintf(QueryStr, AESEncryptEncodingValue("0"), AESEncryptEncodingValue("0"))
		...
		for StatRows.Next() {
			err = StatRows.Scan(&ServOffsetnumstr, &ClntOffsetnumstr)
			if err != nil {
				log.Println("Scan error", err)
				Statistics = nil
				StatRows.Close()
		...
		ServOffsetnumstr = AESDecryptDecodeValue(ServOffsetnumstr)
		ClntOffsetnumstr = AESDecryptDecodeValue(ClntOffsetnumstr)

		ServOffsetnum, _ = strconv.ParseInt(ServOffsetnumstr, 10, 64)
		ClntOffsetnum, _ = strconv.ParseInt(ClntOffsetnumstr, 10, 64)

		QueryStr = "select A.ID, A.Time, A.Bridge_ID_TEXT , A.Proxy_IP_INT, A.Proxy_IP_TEXT, A.Node_IP_INT,\n" +
			"A.Node_IP_TEXT, A.Node_Listen_Port, A.Server_IP_INT, A.Server_IP_TEXT, A.Server_Listen_Port,\n" +
			"B.Client_IP_INT, B.Client_IP_TEXT, B.Inbound, B.Outbound\n" +
			"from SERVER_STATISTICS_COMMON A, SERVER_STATISTICS_DATA B where A.ID = B.ID AND A.ID > %d LIMIT %d"
		QueryStr = fmt.Sprintf(QueryStr, ServOffsetnum, Limitnum)
		log.Println("[LOG] QueryStr : ", QueryStr)
		StatRows, err = sqlitedb_lib.Query_DB(trafficDatabase, QueryStr)
		if err != nil {
			log.Println("Query err", err)
		...
		for StatRows.Next() {
			...
			Statistics = append(Statistics, ServerStatistic)
			ServOffsetnumstr = ServerStatistic.ID
		...
		if len(Statistics) != 0 {
			...
			if err := StatisticsUploadRequest(lwsDatabase, url, userstlbData, authData, authkey, authtoken, Statistics); err != nil {
				log.Println(err)
				Statistics = nil
				StatRows.Close()
				continue
			...
			ServOffsetnumstr = AESEncryptEncodingValue(ServOffsetnumstr)
			...
			_, err = stmt.Exec(ServOffsetnumstr)
			if err != nil {
				stmt.Close()
				sqlitedb_lib.DB_Rollback(tx)
				log.Println("Exec Fail!:", err)
		...
		/*
				...
				"from Client_Statistics_Common A, Client_Statistics_Data B where A.ID = B.ID OFFSET %d "
				QueryStr = fmt.Sprintf(QueryStr, ClntOffsetnum)
			...
			for idx := range Statistics {
				...
				if len(SendStatistics) == 100 {
					...
					ClntOffsetnum += len(SendStatistics)
					SendStatistics = nil
				} else if len(SendStatistics) != 100 && len(SendStatistics) > 0 {
					authkey, authtoken, err := StatisticsAuthRequest(lwsDatabase, url, authData)
					if err != nil {
		...
		*/
		...
		QueryStr = "select A.ID, A.Time, A.Node_ID_TEXT , A.Client_IP_INT, A.Client_IP_TEXT, A.Node_IP_INT,\n" +
			"A.Node_IP_TEXT, A.Node_Listen_Port, B.Proxy_IP_INT, B.Proxy_IP_TEXT, B.Proxy_Listen_Port,\n" +
			"B.Inbound, B.Outbound\n" +
			"from Client_Statistics_Common A, Client_Statistics_Data B where A.ID = B.ID AND A.ID > %d LIMIT %d "
		QueryStr = fmt.Sprintf(QueryStr, ClntOffsetnum, Limitnum)
		log.Println("[LOG] QueryStr : ", QueryStr)
		StatRows, err = sqlitedb_lib.Query_DB(trafficDatabase, QueryStr)
		if err != nil {
			log.Println("Query err", err)
		...
		for StatRows.Next() {
			...
			ServerStatistic.Type = "002"
			Statistics = append(Statistics, ServerStatistic)
			ClntOffsetnumstr = ServerStatistic.ID
		...
		if len(Statistics) != 0 {
			...
			ClntOffsetnumstr = AESEncryptEncodingValue(ClntOffsetnumstr)
			tx, err = sqlitedb_lib.DB_Begin(trafficDatabase)
			if err != nil {
				log.Println("Transaction Begin err:", err)
			}
			...
			_, err = stmt.Exec(ClntOffsetnumstr)
			if err != nil {
				log.Println("Exec Fail!:", err)
				sqlitedb_lib.DB_Rollback(tx)
			}
...
func WebServer_Provisioning_Upload(w http.ResponseWriter, req *http.Request, db *sql.DB) {
  ...
  // kwgwak77@innogs.com date: 2020.09.01, desc: fix cycle time { #0000
  cfginfo.Statistic_Send_Cycle = "60"
  // kwgwak77@innogs.com date: 2020.09.01, desc: fix cycle time { #0000
...
/*
func ProvisioningUpload() error {
	cfgdata, err := ioutil.ReadFile("./cfg/app.cfg")
	if err != nil {
		log.Println("[Error] ProvisioningUpload:", err)
		return err
	}
	...
	var settinginfo SettingsInformation
	settinginfo.Password = ""
	settinginfo.VerifyingPassword
	settinginfo.Maximum_ConnectionCount
	settinginfo.Recv_Buf_Size
	settinginfo.Send_Buf_Size
	settinginfo.Connection_Timeout
	settinginfo.Client_Reconnect_Timeout
	settinginfo.Server_Reconnect_Timeout
	settinginfo.Limit_Size_Log_Storage
	settinginfo.Maxsize_Per_Logfile
	settinginfo.Logfile_Path
	settinginfo.Err_Logfile_Path
	settinginfo.Statistic_Send_Control_Server
	settinginfo.Statistic_Collection_Cycle
	settinginfo.Statistic_Server_Ip
	settinginfo.Statistic_Server_Port
	settinginfo.Statistic_Send_Cycle
	settinginfo.Bridge_Used
	settinginfo.Node_BUFF_SIZE
	settinginfo.Encrypt_Mode
	settinginfo.Change_Client_IP
	settinginfo.Node_ID
	settinginfo.KMS_Address
	settinginfo.KMS_Port

	if cfginfo.Node.Encrypt == "none" {
		settinginfo.Encrypt_Mode = ""
	} else if cfginfo.Node.Encrypt == "aes128" {
		settinginfo.Encrypt_Mode = "aes_128"
	} else if cfginfo.Node.Encrypt == "aes256" {
		settinginfo.Encrypt_Mode = "aes_256"
	} else if cfginfo.Node.Encrypt == "rc4" {
		settinginfo.Encrypt_Mode = "rc4"
	}
	settinginfo.SiteList
