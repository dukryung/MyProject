...
func Create_DB(DbName string) (*sql.DB, error) {
	...
	if err != nil {
		log.Println("Create_DB Error", err)
		return nil, err
	}
	return db, nil
}
...
func Create_Table(db *sql.DB, TableInfo string) (int64, error) {
	...
	if err != nil {
		log.Println("Satement Exec error:", err)
		return DB_RET_FAIL, err
	}
...
func Drop_Table(db *sql.DB, TableName string) (int64, error) {
	var statement *sql.Stmt
	var err error
	var QueryStr string
	...
	if err != nil {
		log.Println("Drop %s error:", TableName, err)
		return DB_RET_FAIL, err
	}
...
func DB_Auto_Commit_Enable(db *sql.DB) error {
	...
	stmt, err = db.Prepare("END TRANSACTION;")
	defer func() {
		if stmt != nil {
			stmt.Close()
		}
