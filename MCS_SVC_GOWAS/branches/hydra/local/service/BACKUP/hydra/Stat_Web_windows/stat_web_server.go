...
var ProcessLogFileName = "./log/stat_web_server.log"
...
func UpdateConfigFiles(Database *sql.DB, Settings SettingsInformation, SeqNo int64) (int32, error) {
	...
	Settings.Statistic_Send_Control_Server = "Enable"
...
func ProvisioningDownloadLocalPorcess(db *sql.DB) {
	...
	for {
		...
		log.Println("here is in ProvisioningDownloadLocalPorcess")
...
func StatisticsUploadLocalProcess(lwsDatabase *sql.DB, trafficDatabase *sql.DB) {
	...
	var ServOffsetCount, ClntOffsetCount int64
	Limitnum = 100
	StatCycletime = 60
	for {
		select {
		...
		*/
		...
		QueryStr = "select A.ID, A.Time, A.Bridge_ID_TEXT , A.Proxy_IP_INT, A.Proxy_IP_TEXT, A.Node_IP_INT,\n" +
			"A.Node_IP_TEXT, A.Node_Listen_Port, A.Server_IP_INT, A.Server_IP_TEXT, A.Server_Listen_Port,\n" +
			"B.Client_IP_INT, B.Client_IP_TEXT, B.Inbound, B.Outbound\n" +
			"from SERVER_STATISTICS_COMMON A, SERVER_STATISTICS_DATA B where A.ID = B.ID LIMIT %d OFFSET %d"
		QueryStr = fmt.Sprintf(QueryStr, Limitnum, ServOffsetnum)
		StatRows, err = sqlitedb_lib.Query_DB(trafficDatabase, QueryStr)
		if err != nil {
			log.Println("Query err", err)
			continue
		...
		*/
		...
		QueryStr = "select A.ID, A.Time, A.Node_ID_TEXT , A.Client_IP_INT, A.Client_IP_TEXT, A.Node_IP_INT,\n" +
			"A.Node_IP_TEXT, A.Node_Listen_Port, B.Proxy_IP_INT, B.Proxy_IP_TEXT, B.Proxy_Listen_Port,\n" +
			"B.Inbound, B.Outbound\n" +
			"from Client_Statistics_Common A, Client_Statistics_Data B where A.ID = B.ID LIMIT %d OFFSET %d "
		QueryStr = fmt.Sprintf(QueryStr, Limitnum, ClntOffsetnum)
		StatRows, err = sqlitedb_lib.Query_DB(trafficDatabase, QueryStr)
		if err != nil {
			log.Println("Query err", err)
			continue
...
func InitLogger() {
	logStruct := &lumberjack.Logger{
		...
		MaxSize:    1,                  // MaxSize is the maximum size in megabytes of the log file before it gets rotated
...
func (p *program) Start() error {
	...
	if DaemonFlag == 1 {
		context := &daemon.Context{
			...
			WorkDir:     "./",
			Umask:       027,
			Args:        []string{"./stat_web_server", WebServerFlag, "-p", bindPort, "-d"},
		}
