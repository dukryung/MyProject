...
var trialgowasasip = "mcstrial.uxcloud.net"
var trialgowasasport = "9091"
var setupgowasasip = "mcssetup.uxcloud.net"
var setupgowasasport = "9092"
var SVCAuthCode string
...
var TrialTranMsgSeq int
var SVCTranMsgSeq int
var TrafficTranMsgSeq int
var TrafficSendCycTime int
...
type SaveTrialID struct {
	TrialID       string `"json:"trialid"`
	TrialAuthCode string `"json:"trialauthcode"`
}

type SaveSVCInfo struct {
	Userkey     string `"json:"userkey"`
	Nodeid      string `"json:"nodeid"`
	GoWASAddr   string `"json:"gowasaddr"`
	UpGoWASAddr string `"json:"upgowasaddr"`
...
//-------------------version Selectinfo-------------------
type VerSelectPageInfo struct {

	//--- trial -----------------{
	TR_Version              string
	TR_Method               string
	TR_SessionType          string
	TR_Seperator            string
	TR_MessageType          string
	TR_MsgSeq               string
	TR_SN1                  string
	TR_SN2                  string
	TR_SN3                  string
	TR_ControlServerAddress string
	TR_SVCtype              string
	TR_Scenariostate        string
	DBAuthCode              string
	//--- trial -----------------}

}
type SVCLoginPageInfo struct {

	//--- trial -----------------{
	SL_Version              string
	SL_Method               string
	SL_SessionType          string
	SL_Seperator            string
	SL_MessageType          string
	SL_MsgSeq               string
	SL_ID                   string
	SL_Ordernum             string
	SL_ControlServerAddress string
	SL_SVCtype              string
	SL_Scenariostate        string
	//--- trial -----------------}
...
type jsonTrialSetupResultValue struct {
	Code    string `json:"code"`
	Message string `json:"message"`
}

func WebServer_Trial_Auth(w http.ResponseWriter, req *http.Request, database *sql.DB) {
	var err error
	var VerSelPageInfo VerSelectPageInfo
	var svctype, scenariostate string
	var mid, mac, setup string
	var macindex int
	var OutputData jsonTrialSetupResultValue
	...
	log.Println("GoWEB Server", req.Method)

	//-------get machineid address--------
	mid, err = machineid.ID()
	if err != nil {
		log.Println("create mid err")
	}
	mid = AESEncryptEncodingValue(mid)
	//-------get machineid address-------

	//-------get mac address-------------
	maclist, err := getMacAddr()
	if err != nil {
		log.Println("Create  mac err")
	}
	for _, a := range maclist {
		mac += a + ","
		macindex++
		if macindex >= 10 {
			break
	...
	mac = AESEncryptEncodingValue(mac)
	svctype, scenariostate, err = SelectVersionCheck(database)
	if err != nil {
		log.Println("version select error")
	}
	setup = "enable"
	setup = AESEncryptEncodingValue(setup)

	VerSelPageInfo.TR_SVCtype = svctype
	VerSelPageInfo.TR_Scenariostate = scenariostate
	VerSelPageInfo.DBAuthCode = Check_Code_Checksum("TrialAuthCode", "TrialAuthCodeTbl")

	trialgowasaddr := trialgowasasip + ":" + trialgowasasport
	url := fmt.Sprintf("http://%s/auth_api/trial_setup/v1.0/", trialgowasaddr)

	authkey, authtoken, err := TrialSetupAuthRequest(url, mid, mac, setup)
	if err != nil {
		log.Println("[ERR] : ", err)
		OutputData.Code = Check_Code_Checksum("TrialAuthCode", "TrialAuthCodeTbl")

		if OutputData.Code == "700" {
			log.Println("[ERR] : Take plage Network problem")
			OutputData.Message = "Take plage Network problem"
		} else if OutputData.Code == "652" {
			log.Println("[ERR] : Expired Period of Trial Version")
			OutputData.Message = "Expired Period of Trial Version"
		} else if OutputData.Code == "653" {
			log.Println("[ERR] : Limited Trial Service")
			OutputData.Message = "Limited Trial Service"
		} else if OutputData.Code == "654" {
			log.Println("[ERR] : Aleady Trial Service")
			OutputData.Message = "Aleady Trial Service"
		} else {
			log.Println("[ERR] : Can't Operation MCSE")
			OutputData.Message = "Can't Operation MCSE"
		}
	...
	if err := TrialSetupInfoRequest(database, url, mid, mac, setup, authkey, authtoken); err != nil {
		log.Println("[ERR] : ", err)
		OutputData.Code = Check_Code_Checksum("TrialAuthCode", "TrialAuthCodeTbl")
...
func TrialSetupInfoRequest(database *sql.DB, url string, SN1 string, SN2 string, SN3 string, authkey string, authtoken string) error {
	var authReqProto jsonInputWebAPIAuthTrialSetupPack

	TrialTranMsgSeq++
	if TrialTranMsgSeq >= 100000 {
		TrialTranMsgSeq = 1
	}

	authReqProto.Version = "1.0"
	authReqProto.Method = "auth"
	authReqProto.SessionType = "register"
	authReqProto.Seperator = "setup"
	authReqProto.MessageType = "request"
	authReqProto.MessageSeq = strconv.Itoa(TrialTranMsgSeq)
	authReqProto.SN1 = SN1
	authReqProto.SN2 = SN2
	authReqProto.SN3 = SN3
	...
	for {
		...
		if err != nil {
			...
			log.Println("[ERR] : ", err)
			if retrynum == 3 {
				SqlitedbUpdateCode("TrialAuthCodeTbl", "TrialAuthCode", "700")
				return err
			}
		} else {
			if resp.StatusCode != 200 {
				...
				if retrynum == 3 {
					return errors.New(fmt.Sprintf("Response Auth Trial Setup Http error: %d", resp.Status))
				}
			} else {
	...
	authResProto := jsonOutputWebAPIAuthTrialSetup{}
	if err := json.Unmarshal(bodyBytes, &authResProto); err != nil {
		log.Println("[ERR] : ", err)
		return errors.New(err.Error() + "\nurl: " + url)
	}
	log.Println("[LOG] Code : ", authResProto.Code)
	log.Println("[LOG] Message : ", authResProto.Message)
	log.Println("[LOG] Message : ", string(bodyBytes))

	if authResProto.Code != "200" {
		log.Println("[ERR] Code : ", authResProto.Code)
		log.Println("[ERR] Message : ", authResProto.Message)
		SqlitedbUpdateCode("TrialAuthCodeTbl", "TrialAuthCode", authResProto.Code)
		return errors.New("failed auth request")
	}

	if authResProto.Version != "1.0" {
		log.Println("[ERR] Version Not Match : ", authResProto.Version)
		return errors.New("invailed auth protocol Version")
	}
	if authResProto.Method != authReqProto.Method {
		log.Println("[ERR] Method Not Match : ", authResProto.Method)
		return errors.New("invailed auth protocol Method")
	}
	if authResProto.MessageType != "response" {
		log.Println("[ERR] MsgType Not Match : ", authResProto.MessageType)
		return errors.New("invailed auth protocol MsgType")
	}
	if authResProto.SessionType != authReqProto.SessionType {
		log.Println("[ERR] SessionType Not Match : ", authResProto.SessionType)
		return errors.New("invailed auth protocol SessionType")
	}

	err = Save_TrialID_Info(database, authResProto.Param, authResProto.Code)
	if err != nil {
		log.Println("[ERR] save svc info : ", err)
		return err
	}
...
func Save_TrialID_Info(database *sql.DB, TrialIDInfo string, Code string) error {
	var err error
	var fd *os.File
	var statusfd *os.File
	var HashingText, salt, authkey, trialid, checksum, comparechecksum, trialdhashingvalue string
	var hashing_algorithm hash.Hash
	var arrchecksum []string
	var DecryptTrialChecksum string
	var status, statussalt, statusexpiretime, statuschecksum string
	var nowtime time.Time
	var QueryStr string
	var Rows *sql.Rows
	var rowscnt int

	DecryptTrialChecksum = AESDecryptDecodeValue(TrialIDInfo)
	arrchecksum = strings.Split(DecryptTrialChecksum, ":")
	if len(arrchecksum) != 3 {
		log.Println("[ERR] Invalid trialidinfo")
		return errors.New("[ERR] Invalid checksum")
	}
	authkey = arrchecksum[0]
	trialid = arrchecksum[1]
	checksum = arrchecksum[2]

	hashing_algorithm = md5.New()
	HashingText = authkey + ":" + trialid
	hashing_algorithm.Write([]byte(HashingText))
	comparechecksum = hex.EncodeToString(hashing_algorithm.Sum(nil))
	log.Println("authkey:", authkey)
	log.Println("trialid:", trialid)
	log.Println("comparechecksum:", comparechecksum)
	log.Println("checksum:", checksum)

	if comparechecksum != checksum {
		log.Println("[ERR] Invalid checksum")
		return errors.New("[ERR] Invalid checksum")
	}

	if _, err = os.Stat("./cfg/trialid.key"); os.IsNotExist(err) {
		log.Println("File not exist! -> ", "./cfg/trialid.key")
	} else {
		log.Println("prev File exist! -> ", "./cfg/trialid.key")
		err = os.Remove("./cfg/trialid.key")
		if err != nil {
			log.Println("Remove err:", err)
			return errors.New("[ERR] trialid.key file remove error")
		}
	}
	fd, err = os.Create("./cfg/trialid.key")
	if err != nil {
		log.Println("create trialid file err:", err)
		return errors.New("[ERR] create trialid.key error")
	}
	defer fd.Close()

	salt = "trial_certification_identity"
	hashing_algorithm = md5.New()
	HashingText = salt + ":" + trialid
	hashing_algorithm.Write([]byte(HashingText))
	trialdhashingvalue = hex.EncodeToString(hashing_algorithm.Sum(nil))

	_, err = fd.Write([]byte(AESEncryptEncodingValue(salt + ":" + trialid + ":" + trialdhashingvalue)))
	if err != nil {
		log.Println(" Write err:", err)
		return errors.New("[ERR] create trialid.key error")
	}

	if _, err = os.Stat("./cfg/status.key"); os.IsNotExist(err) {
		log.Println("File not exist! -> ", "./cfg/status.key")

	} else {
		log.Println("prev File exist! -> ", "./cfg/status.key")
	}

	statusfd, err = os.Create("./cfg/status.key")
	if err != nil {
		log.Println(" create status file err:", err)
		return errors.New("[ERR] create trialid.key error")
	}

	defer statusfd.Close()

	statussalt = "permission_status"
	status = "start"
	nowtime = time.Now()
	statusexpiretime = nowtime.Add(time.Second * 30).Format("2006-01-02 15:04:05")

	hashing_algorithm = md5.New()
	HashingText = statussalt + "|" + status
	hashing_algorithm.Write([]byte(HashingText))
	statuschecksum = hex.EncodeToString(hashing_algorithm.Sum(nil))

	_, err = statusfd.Write([]byte(AESEncryptEncodingValue(statussalt + "|" + status + "|" + statusexpiretime + "|" + statuschecksum)))
	if err != nil {
		log.Println(" Write err:", err)
		return errors.New("[ERR] create trialid.key error")
	}

	QueryStr = "select count(*)  from VersionModeTbl"
	...
	if rowscnt == 0 {
		InsertVersionCheck("trial", "trial_certified", database)
	} else {
		UpdateVersionCheck("trial", "trial_certified", database)
	}

	SqlitedbUpdateCode("TrialAuthCodeTbl", "TrialAuthCode", Code)

	return nil

}

func TrialSetupAuthRequest(url string, SN1 string, SN2 string, SN3 string) (string, string, error) {
	var authReqProto jsonInputWebAPIAuthTrialSetupPack
	...
	for {
		...
		if err != nil {
			...
			if resp.StatusCode != 200 {
				...
				log.Println("[LOG] resp.StatusCode : ", resp.StatusCode)
				if retrynum == 3 {
					return "", "", errors.New(fmt.Sprintf("Response Auth Trial Http error: %d", resp.Status))
				}
			} else {
				log.Println("[LOG] : Success client do")
				break
	...
	if resp.StatusCode != 200 {
		SqlitedbUpdateCode("TrialAuthCodeTbl", "TrialAuthCode", "700")
		return "", "", errors.New(fmt.Sprint("Response Auth Trial Setup Http error: %d", resp.Status))
	}
	...
	log.Println("[LOG] Code : ", authResProto.Code)
	log.Println("[LOG] Message : ", authResProto.Message)
	log.Println("[LOG] AuthInfo json : ", string(bodyBytes))
	...
	if authResProto.MessageType != "response" {
		log.Println("[ERR] MsgType Not Match : ", authResProto.MessageType)
		return "", "", errors.New("invailed auth protocol Message")
	}
	if authResProto.SessionType != authReqProto.SessionType {
		log.Println("[ERR] SessionType Not Match : ", authResProto.SessionType)
		return "", "", errors.New("invailed auth protocol SessionType")
	...
	if authResProto.AuthKey == "" {
		log.Println("[ERR] AuthKey Empty : ", authResProto.AuthKey)
		return "", "", errors.New("invailed auth protocol AuthKey")
	}

	AuthToken := Trial_Setup_Make_AuthToken(SN1, SN2, authReqProto.Method, authResProto.AuthKey)

	return authResProto.AuthKey, AuthToken, nil
}

func Trial_Setup_Make_AuthToken(SN1 string, SN2 string, Method string, AuthKey string) string {

	log.Println("SN1:", SN1)
	log.Println("SN2:", SN2)

	DecryptSN1 := AESDecryptDecodeValue(SN1)
	DecryptSN2 := AESDecryptDecodeValue(SN2)
	log.Println("DecryptSN1:", DecryptSN1)
	log.Println("DecryptSN2:", DecryptSN2)
	...
	hashing_algorithm = md5.New()
	HashingText = Method + ":" + "/auth_api/trial_setup/v1.0/"
	hashing_algorithm.Write([]byte(HashingText))
	HA2 := hex.EncodeToString(hashing_algorithm.Sum(nil))

	hashing_algorithm = md5.New()
	HashingText = HA1 + ":" + AuthKey + ":" + HA2
	hashing_algorithm.Write([]byte(HashingText))
...
func WebServer_Ver_Select(w http.ResponseWriter, req *http.Request, database *sql.DB) {
	...
	/*
		if runtime.GOOS == "linux" {
			...
			if _, err := os.Stat("/etc/setup.key"); os.IsNotExist(err) {
				log.Println("not exist setup.key")
				fd, err := os.Create("/etc/mcs/setup.key")
				if err != nil {
					log.Println("key Create err")
					return
				}

				_, err = fd.Write([]byte("yes"))
				if err != nil {
					log.Println(" Write err:", err)
					return
				}
			} else {
				log.Println("exist setup.key")
				setupinfo, err := ioutil.ReadFile("/etc/setup.key")
				if err != nil {
					log.Println(err)
				}
				setup = AESEncryptEncodingValue(string(setupinfo))
			}

		} else if runtime.GOOS == "windows" {

			k, err := registry.OpenKey(registry.LOCAL_MACHINE, `Software\Microsoft\Windows`, registry.QUERY_VALUE|registry.SET_VALUE)
			if err != nil {
				log.Println("Open Key error")
			}
			if setup, _, err = k.GetStringValue("setup"); err != nil {
				log.Println("not exist setup!!")
				if err := k.SetStringValue("setup", "yes"); err != nil {
					log.Println("Set setup value fail")
				} else {
					if setup, _, err = k.GetStringValue("setup"); err != nil {
						log.Println("Get setup value fail")
					}
				}
			}
	...
	*/
	...
	VerSelPageInfo.TR_Version = "1.0"
	VerSelPageInfo.TR_Method = "auth"
	VerSelPageInfo.TR_SessionType = "register"
	VerSelPageInfo.TR_Seperator = "setup"
	VerSelPageInfo.TR_MessageType = "request"
	VerSelPageInfo.TR_MsgSeq = strconv.Itoa(TrialTranMsgSeq)
	VerSelPageInfo.TR_SN1 = mid
	VerSelPageInfo.TR_SN2 = mac
	VerSelPageInfo.TR_SN3 = setup
	VerSelPageInfo.TR_ControlServerAddress = trialgowasasip + ":" + trialgowasasport
	VerSelPageInfo.TR_SVCtype = svctype
	VerSelPageInfo.TR_Scenariostate = scenariostate
	VerSelPageInfo.DBAuthCode = Check_Code_Checksum("TrialAuthCode", "TrialAuthCodeTbl")
	//---------------- insert  logic  must check  to be prevtrial -------------

	tmpl, err = template.ParseFiles("./pages/Trial_Selection.html")
	if err != nil {
	...
	tmpl.Execute(w, VerSelPageInfo)
}

func WebServer_Trial_Update(w http.ResponseWriter, req *http.Request, database *sql.DB, trafficdatabase *sql.DB) {
	...
	Drop_Traffic_Table(trafficdatabase)
	...
	cmd := exec.Command(updateexe, svctype, updategowasaddr, osversion)
	err = cmd.Start()
	if err != nil {
		log.Println(fmt.Sprint(err))
		WebServer_Redirect(w, req, "/versionselect/")
		return
	}
...
func Drop_Traffic_Table(Database *sql.DB) {
	var DB_Flag int64

	DB_Flag, _ = sqlitedb_lib.Drop_Table(Database, "Server_Statistics_Common")
	if DB_RET_FAIL == DB_Flag {
		log.Println("Drop Table Fail!")
	}
	DB_Flag, _ = sqlitedb_lib.Drop_Table(Database, "Server_Statistics_Data")
	if DB_RET_FAIL == DB_Flag {
		log.Println("Drop Table Fail!")
	}
	DB_Flag, _ = sqlitedb_lib.Drop_Table(Database, "Client_Statistics_Common")
	if DB_RET_FAIL == DB_Flag {
		log.Println("Drop Table Fail!")
	}
	DB_Flag, _ = sqlitedb_lib.Drop_Table(Database, "Client_Statistics_Data")
	if DB_RET_FAIL == DB_Flag {
		log.Println("Drop Table Fail!")
	}
	DB_Flag, _ = sqlitedb_lib.Drop_Table(Database, "clienttrafficinfo")
	if DB_RET_FAIL == DB_Flag {
		log.Println("Drop Table Fail!")
	}
	DB_Flag, _ = sqlitedb_lib.Drop_Table(Database, "servertrafficinfo")
	if DB_RET_FAIL == DB_Flag {
		log.Println("Drop Table Fail!")
	}
...
type SVCLoginInfo struct {
	LoginID  string `json:"login_id"`
	Password string `json:"password"`
	OrderNum string `json:"order_num"`
}
type jsonSetupResultValue struct {
	Code    string `json:"code"`
	Message string `json:"message"`
}

func WebServer_Logging_SVC(w http.ResponseWriter, req *http.Request, database *sql.DB, trafficdatabase *sql.DB) {
	var InputData SVCLoginInfo
	var OutputData jsonSetupResultValue
	var OutputBody string
	var err error
	log.Println("[LOG] InputData :", InputData)
	...
	if InputData.LoginID == "" || InputData.OrderNum == "" || InputData.Password == "" {
	...
	setupgowasaddr := setupgowasasip + ":" + setupgowasasport

	url := fmt.Sprintf("http://%s/auth_api/svc_setup/v1.0/", setupgowasaddr)
	authkey, authtoken, err := SVCSetupAuthRequest(url, InputData.LoginID, InputData.OrderNum, InputData.Password)
	if err != nil {
		log.Println("[ERR] : ", err)
		if SVCAuthCode == "" {
			OutputData.Code = "600"
		} else {
			OutputData.Code = SVCAuthCode
		}
	...
	if err := SVCSetupInfoRequest(database, trafficdatabase, url, InputData.LoginID, InputData.Password, InputData.OrderNum, authkey, authtoken); err != nil {
		log.Println("[ERR] : ", err)
		if SVCAuthCode == "" {
			OutputData.Code = "600"
		} else {
...
func SVCSetupInfoRequest(database *sql.DB, trafficdatabase *sql.DB, url string, ID string, Password string, OrderNum string, authkey string, authtoken string) error {
	var authReqProto jsonInputWebAPIAuthSvcSetupPack
	SVCTranMsgSeq++
	if SVCTranMsgSeq >= 100000 {
		SVCTranMsgSeq = 1
	}
	...
	authReqProto.Method = "auth"
	authReqProto.SessionType = "register"
	authReqProto.Seperator = "init_setup"
	authReqProto.MessageType = "request"
	authReqProto.MsgSeq = strconv.Itoa(SVCTranMsgSeq)
	authReqProto.LoginID = AESEncryptEncodingValue(ID)
	authReqProto.LoginPW = AESEncryptEncodingValue(Password)
	authReqProto.OrderNum = AESEncryptEncodingValue(OrderNum)
	authReqProto.CheckSum = SVC_Checksum(ID, OrderNum)
	authReqProto.AuthKey = authkey
	authReqProto.AuthToken = authtoken
	...
	for {
		...
		if err != nil {
			...
			if resp.StatusCode != 200 {
				...
				log.Println("[LOG] resp.StatusCode : ", resp.StatusCode)
				if retrynum == 3 {
					return errors.New(fmt.Sprintf("Response Auth Service Setup Http error: %d", resp.Status))
				}
			} else {
				log.Println("[LOG] : Success client do")
				break
	...
	bodyBytes, _ := ioutil.ReadAll(resp.Body)
	authResProto := jsonOutputWebAPIAuthSVCSetup{}
	if err := json.Unmarshal(bodyBytes, &authResProto); err != nil {
		log.Println("[ERR] : ", err)
		return errors.New(err.Error() + "\nurl: " + url)
	}
	...
	err = WebServer_Save_SVCInfo(database, authResProto.ParamUserKey, authResProto.ParamNodeID, authResProto.ParamServiceGoWas, authResProto.ParamUpdateGoWas, authResProto.ParamCKSum)
	if err != nil {
		log.Println("[ERR] save svc info : ", err)
		return err
	}

	err = Excute_Trial_Update(trafficdatabase)
	if err != nil {
		log.Println("[ERR] Excute Trial Update : ", err)
		return err
	}
...
func Excute_Trial_Update(trafficdatabase *sql.DB) error {
	var updateexe, svctype, osversion string
	var err error
	...
	if updateExtime.IsZero() == true {
		...
		if time.Now().After(updateExtime) == true {
			updateExtime = time.Now().Add(time.Minute * 3)
			log.Println("[LOG] : failed updating")
			log.Println("[LOG] : start updating")
		} else {
			log.Println("[LOG] : is updating")
			return nil
		}
	}
	gowasaddr, updategowasaddr, err := GetGoWasAddr()
	...
	err = cmd.Start()
	if err != nil {
		log.Println("[ERR] exec command : ", err)
		return err
	}
...
func SVCSetupAuthRequest(url string, ID string, OrderNum string, Password string) (string, string, error) {
	var authReqProto jsonInputWebAPIAuthSvcSetupPack
	SVCTranMsgSeq++
	if SVCTranMsgSeq >= 100000 {
		SVCTranMsgSeq = 1
	...
	if resp.StatusCode != 200 {
		log.Println("[ERR] : ", err)
		return "", "", errors.New(fmt.Sprint("Response Auth Service Setup Http error: %d", resp.Status))
	}
	...
	AuthToken := SVC_Setup_Make_AuthToken(ID, authReqProto.Method, authResProto.AuthKey, Password)

	return authResProto.AuthKey, AuthToken, nil

}

func WebServer_Save_SVCInfo(database *sql.DB, UserKey string, NodeID string, GoWASAddr string, UpGoWASAddr string, InputChecksum string) error {
	var err error
	var userkeyfd, nodeidfd, gowasaddrfd *os.File
	var ConfUserkey, ConfNodeID, ConfGOWASAddr string
	var EncText string
	var CRLF string
	var userkey, nodeid, inputchecksum string
	var hashing_algorithm hash.Hash
	var HashingText string
	var Rows *sql.Rows
	var QueryStr string
	var rowscnt int
	var gowasaddr, upgowasaddr string
	var arrchecksum []string
	var comparechecksum string

	//------------validation check----------------------------
	userkey = AESDecryptDecodeValue(UserKey)
	nodeid = AESDecryptDecodeValue(NodeID)
	inputchecksum = AESDecryptDecodeValue(InputChecksum)
	gowasaddr = AESDecryptDecodeValue(GoWASAddr)
	upgowasaddr = AESDecryptDecodeValue(UpGoWASAddr)

	arrchecksum = strings.Split(inputchecksum, ":")
	if len(arrchecksum) != 3 {
		log.Println("[ERR] : Invalid arrchecksum")
		return err
	}
	comparechecksum = arrchecksum[2]

	hashing_algorithm = md5.New()
	HashingText = userkey + ":" + nodeid
	hashing_algorithm.Write([]byte(HashingText))
	Checksum := hex.EncodeToString(hashing_algorithm.Sum(nil))

	if Checksum != comparechecksum {
		log.Println("[ERR] : SVC Invalid checksum")
		return err
	}
	//------------validation check----------------------------
	if runtime.GOOS == "linux" {
	...
	ConfUserkey += "[UserKey]" + CRLF
	ConfUserkey += "UserKey = \"<UserKey>\"" + CRLF
	ConfUserkey += CRLF
	...
	ConfGOWASAddr += "[GOWASADDR]" + CRLF
	ConfGOWASAddr += "GOWASADDR = \"<GOWASADDR>\"" + CRLF
	ConfGOWASAddr += CRLF

	ConfGOWASAddr += "[UPGOWASADDR]" + CRLF
	ConfGOWASAddr += "UPGOWASADDR = \"<UPGOWASADDR>\"" + CRLF
	ConfGOWASAddr += CRLF

	ConfUserkey = strings.Replace(ConfUserkey, "<UserKey>", userkey, -1)
	ConfNodeID = strings.Replace(ConfNodeID, "<NODE_ID>", nodeid, -1)
	ConfNodeID = strings.Replace(ConfNodeID, "<UserKeyID>", "", -1)
	ConfNodeID = strings.Replace(ConfNodeID, "<DeviceID>", "", -1)
	ConfGOWASAddr = strings.Replace(ConfGOWASAddr, "<GOWASADDR>", gowasaddr, -1)
	ConfGOWASAddr = strings.Replace(ConfGOWASAddr, "<UPGOWASADDR>", upgowasaddr, -1)
	log.Println("ConfGOWASAddr:", ConfGOWASAddr)

	userkeyfd, err = os.Create("./cfg/userkey.key")
	if err != nil {
		return err
	}

	EncryptEncodingStr(ConfUserkey, &EncText)
	_, err = userkeyfd.Write([]byte("COD$_"))
	if err != nil {
		log.Println("[ERR] Write err:", err)
		return err
	}

	_, err = userkeyfd.Write([]byte(EncText))
	if err != nil {
		log.Println("[ERR] Write err:", err)
		return err
	}
	defer userkeyfd.Close()

	nodeidfd, err = os.Create("./cfg/nodeid.key")
	if err != nil {
		return err
	}

	EncryptEncodingStr(ConfNodeID, &EncText)
	_, err = nodeidfd.Write([]byte("COD$_"))
	if err != nil {
		log.Println("[ERR] Write err:", err)
		return err
	}

	_, err = nodeidfd.Write([]byte(EncText))
	if err != nil {
		log.Println("[ERR] Write err:", err)
		return err
	}
	defer nodeidfd.Close()

	gowasaddrfd, err = os.Create("./cfg/addr.dat")
	if err != nil {
		return err
	}

	EncryptEncodingStr(ConfGOWASAddr, &EncText)
	_, err = gowasaddrfd.Write([]byte("COD$_"))
	if err != nil {
		log.Println("[ERR] Write err:", err)
		return err
	}

	_, err = gowasaddrfd.Write([]byte(EncText))
	if err != nil {
		log.Println("[ERR] Write err:", err)
		return err
	}
	defer gowasaddrfd.Close()
	...
	if rowscnt == 0 {
		InsertVersionCheck("svc", "trial_update", database)
	} else {
		UpdateVersionCheck("svc", "trial_update", database)
	}
...
func SVC_Setup_Make_AuthToken(ID string, Method string, AuthKey string, Password string) string {

	//log.Println("Password:", Password)
	log.Println("[LOG] ID :", ID)

	hashing_algorithm := md5.New()
	HashingText := ID + ":" + Password
	hashing_algorithm.Write([]byte(HashingText))
	HA1 := hex.EncodeToString(hashing_algorithm.Sum(nil))

	/*
		sha1hasing_algorithm := sha1.New()
		sha1hasing_algorithm.Write([]byte(Password))
		sha1password := hex.EncodeToString(sha1hasing_algorithm.Sum(nil))
		log.Println("Password:", Password)
		log.Println("sha1password:", sha1password)

		HA1 = ID + ":" + sha1password
	*/

	hashing_algorithm = md5.New()
	HashingText = Method + ":" + "/auth_api/svc_setup/v1.0/"
	hashing_algorithm.Write([]byte(HashingText))
	HA2 := hex.EncodeToString(hashing_algorithm.Sum(nil))
...
func SVC_Checksum(ID string, OrderNumber string) string {

	hashing_algorithm := md5.New()
	HashingText := ID + ":" + OrderNumber
	hashing_algorithm.Write([]byte(HashingText))
	Output := hex.EncodeToString(hashing_algorithm.Sum(nil))
	Output = AESEncryptEncodingValue(Output)

	return Output
}
func WebServer_Login_SVC(w http.ResponseWriter, req *http.Request, database *sql.DB) {
	var tmpl *template.Template
	var err error
	var SVCLoginInfo SVCLoginPageInfo
	var rowscnt int
	defer req.Body.Close()
	log.Println("GoWEB Server", req.Method)
	...
	tmpl, err = template.ParseFiles("./pages/Login_MCS.html")
	if err != nil {
		log.Println("failed to template.ParseFiles")
		return
	}

	tmpl.Execute(w, SVCLoginInfo)
}
...
func WebServer_Login_SVC(w http.ResponseWriter, req *http.Request, database *sql.DB) {
	...
	defer Rows.Close()
	log.Println("WebServer_Login_SVC!")
	if rowscnt == 0 {
		InsertVersionCheck("svc", "svc_setup", database)
	} else {
		UpdateVersionCheck("svc", "svc_setup", database)
	}
	...
	SVCLoginInfo.SL_Version = "1.0"
	SVCLoginInfo.SL_Method = "auth"
	SVCLoginInfo.SL_SessionType = "register"
	SVCLoginInfo.SL_Seperator = "init_setup"
	SVCLoginInfo.SL_MessageType = "request"
	SVCLoginInfo.SL_MsgSeq = strconv.Itoa(SVCTranMsgSeq)
	SVCLoginInfo.SL_ControlServerAddress = setupgowasasip + ":" + setupgowasasport
	SVCLoginInfo.SL_SVCtype = svctype
	SVCLoginInfo.SL_Scenariostate = scenariostate

	log.Println("SVCTranMsgSeq", SVCTranMsgSeq)
	tmpl, err = template.ParseFiles("./pages/Login_MCS.html")
	if err != nil {
		log.Println("failed to template.ParseFiles")
		return
...
func WebServer_Save_TrialID(w http.ResponseWriter, req *http.Request, database *sql.DB) {
	var err error
	var fd *os.File
	var statusfd *os.File
	var TrialIDInfo SaveTrialID
	var HashingText, salt, authkey, trialid, checksum, comparechecksum, trialdhashingvalue string
	var hashing_algorithm hash.Hash
	var arrchecksum []string
	var DecryptTrialChecksum string
	...
	err = json.Unmarshal(bodyData, &TrialIDInfo)
	if err != nil {
		log.Println("error TrialIDInfo json parser:", TrialIDInfo)
		goto err_res
	}

	DecryptTrialChecksum = AESDecryptDecodeValue(TrialIDInfo.TrialID)
	arrchecksum = strings.Split(DecryptTrialChecksum, ":")
	if len(arrchecksum) != 3 {
		log.Println("Invalid trialidinfo")
		goto err_res
	}
	authkey = arrchecksum[0]
	trialid = arrchecksum[1]
	...
	if comparechecksum != checksum {
		log.Println("Trial invalid checksum")
		goto err_res
	}
	...
	SqlitedbUpdateCode("TrialAuthCodeTbl", "TrialAuthCode", TrialIDInfo.TrialAuthCode)
...
func WebServer_Login(w http.ResponseWriter, req *http.Request, database *sql.DB) {
	var tmpl *template.Template
	var err error
	var CodeInfo CodeInformation
	defer req.Body.Close()

	Code := Check_Code_Checksum("Code", "CodeTbl")
...
func UpdateConfigFiles(Database *sql.DB, Settings SettingsInformation /*, SeqNo int64*/) (int32, error) {
	...
	if Settings.Password == "" {
		...
		_, err = stmt.Exec(EncryptGoWebPassword(Settings.Password))
		if err != nil {
			stmt.Close()
	...
	log.Println("[LOG] : ", Whole_Config_File)
...
func WebServer_Settings(w http.ResponseWriter, req *http.Request, db *sql.DB) {
	...
	if ControlServerFlag == 0 {
		...
		QueryStr := fmt.Sprintf("SELECT Stat_StatDataSendCycle,Stat_Send_Flag FROM Users;")
		...
		for Rows.Next() {
			err := Rows.Scan(&Control_Server_Send_Interval, &StatSendFlag)
			if err != nil {
				log.Println(" data Scan error:", err)
				return
			}
		...
		SetPageInfo.Control_Server_IP = trialgowasasip
		SetPageInfo.Control_Server_Port, _ = strconv.Atoi(trialgowasasport)
		...
		SetPageInfo.Control_Server_Send_Interval = 3600
		...
		*/
		...
		for Frontendname, Frontendelm = range cfginfo.Frontend {
			...
			if Node_Mode == Node_MODE_NONE {
				...
				OptionStr = fmt.Sprintf("<option value=\"%d\">선택해주세요</option><option value=\"%d\">SML Client</option><option selected=\"selected\" value=\"%d\">Node Server</option>", Node_MODE_NONE, Node_MODE_CLIENT, Node_MODE_SERVER)
			}
			if count == 1 {
			...
			TempStr = fmt.Sprintf("%s<div data-SiteType=\"1\">%s<h2>Frontend</h2><table class=\"input\"><colgroup><col width=\"250\"><col></colgroup><tbody><tr><th>Symbol</th><td><input type=\"text\" class=\"s100\" Frontendsymbol reserve=\"ko en number space length\" min=\"2\" max=\"32\" msg=\"글자 2 - 32 까지만 입력이 가능 합니다.\" group=\"all\" value=\"%s\"></td></tr><tr><th>Bind Port</th><td><input type=\"text\" class=\"s100\" FrontendPort reserve=\"between\" min=\"1\" max=\"65535\" msg=\"숫자 1 - 65535 까지만 입력이 가능 합니다.\" group=\"all\" value=\"%d\"/></td></tr><tr><th>SML Mode</th><td><select Node_Mode>%s</select><button type=\"button\" act=\"btnFrontendConfirm\" >Confirm</button></td></tr></tbody></table>", IDTagStart, HRTag, Symbol_Name, Bind, OptionStr)
			if Node_Mode == Node_MODE_CLIENT {
		...
		NodeModeStr += fmt.Sprintf("<option value=\"%d\">SML Client</option>", Node_MODE_CLIENT)
		if DeviceOSFlag == GENERAL_OS {
			NodeModeStr += fmt.Sprintf("<option value=\"%d\">SML Server</option>", Node_MODE_SERVER)
		}
		...
		if count == 0 {
			...
			TempStr = fmt.Sprintf("<div id=\"Frontend\"><div data-SiteType=\"1\"><h2>Frontend<div></div></h2><table class=\"input\"><colgroup><col width=\"250\"><col></colgroup><tbody><tr><th>Symbol</th><td><input type=\"text\" class=\"s100\" Frontendsymbol reserve=\"ko en number space length\" min=\"2\" max=\"32\" msg=\"글자 2 - 32 까지만 입력이 가능 합니다.\" group=\"all\" value=\"\"></td></tr><tr><th>Bind Port</th><td><input type=\"text\" class=\"s100\" FrontendPort reserve=\"between\" min=\"1\" max=\"65535\" msg=\"숫자 1 - 65535 까지만 입력이 가능 합니다.\" group=\"all\" value=\"\"/></td></tr><tr><th>Node Mode</th><td><select Node_Mode>%s</select><button type=\"button\" act=\"btnFrontendConfirm\" >Confirm</button></td></tr></tbody></table></div></div>", NodeModeStr)
			TempHTML.Value_HTML = template.HTML(TempStr)
			SetPageInfo.FrontBackHTMLList = append(SetPageInfo.FrontBackHTMLList, TempHTML)
		}
		...
		*/
		//-------------------------------------------------------------------------------------------}
		if count > 1 || backendcount > 2 {

			log.Println("frontend or backend count over limit")
			WebServer_Redirect(w, req, "/tr_setting/")
			return
		}
...
type jsonInputWebChecksumEncodeValue struct {
	ID          string `json:"id"`
	OrderNumber string `json:"ordernum"`
}

type jsonOutputWebChecksumEncodeValue struct {
	Code        string `json:"code"`
	Message     string `json:"message"`
	OutputValue string `json:"output"`
}
...
func WebServer_Web_Auth_SVC_Checksum(w http.ResponseWriter, req *http.Request) {
	defer req.Body.Close()
	var InputData jsonInputWebChecksumEncodeValue
	var OutputData jsonOutputWebChecksumEncodeValue
	var OutputBody string
	var err error

	log.Println("WebServer_Web_Auth_SVC_Checksum", req.Method)
	...
	*/
	...
	log.Println("Input ID:" + InputData.ID + " Input Ordernumber:" + InputData.OrderNumber)

	hashing_algorithm := md5.New()
	HashingText := InputData.ID + ":" + InputData.OrderNumber
	hashing_algorithm.Write([]byte(HashingText))
	Output := hex.EncodeToString(hashing_algorithm.Sum(nil))
	Output = AESEncryptEncodingValue(Output)
...
type jsonInputWebAPIAuthSVCTokenPack struct {
	Method      string `json:"method"`
	SessionType string `json:"sessiontype"`
...
type jsonOutputWebAPIAuthSVCTokenPack struct {
	Code        string `json:"code"`
	Message     string `json:"message"`
	InputValue  string `json:"input"`
	MsgSeq      string `json:"msgseq"`
...
func WebServer_Web_Auth_API_SVC_Setup_AuthToken(w http.ResponseWriter, req *http.Request) {
	defer req.Body.Close()
	var InputData jsonInputWebAPIAuthSVCTokenPack
	var OutputData jsonOutputWebAPIAuthSVCTokenPack
	var OutputBody string
	var HashingText string
	var HA1 string
	var HA2 string
	...
	log.Println("WebServer_Web_Auth_API_SVC_Setup_AuthToken", req.Method)
	/*
		res := Cookie_Check(w, req)
		if res < 0 {
			OutputData.Code = "600"
	...
	*/
	...
	log.Println(">>> Input Data : [method:" + InputData.Method + ", sessiontype:" + InputData.SessionType + ", authkey:" + InputData.AuthKey + "]")

	if InputData.Method == "" || InputData.SessionType == "" || InputData.AuthKey == "" || InputData.ID == "" || InputData.Password == "" {
		log.Println("invalid parmeter value: null")
		OutputData.Code = "600"
		OutputData.Message = "parmeter value is null"
		OutputData.InputValue = ""
	...
	if InputData.Method != "auth" || InputData.SessionType != "register" {
	...
	ID := AESDecryptDecodeValue(InputData.ID)
	Password := AESDecryptDecodeValue(InputData.Password)
	...
	*/
	...
	if Response != "" {
		...
		OutputData.InputValue = ""
		OutputData.MsgSeq = strconv.Itoa(SVCTranMsgSeq)
		...
		log.Printf("web api response  [code:%s, msg:%s, description:%s (authkey:%s, authtoken:%s)]", OutputData.Code, OutputData.Message, "authtoken", InputData.AuthKey, Response)
		return
...
func WebServer_Client_Statistics(w http.ResponseWriter, req *http.Request, Database *sql.DB) {
	...
	if ControlServerFlag == 0 {
		...
		TempStr = fmt.Sprintf("<th><a href=\"/statistics/client/?page_num=1&sort=%d%s\">SML Client IP %s</a></th>", SortNodeIP, Params, SortNodeIPDir)
		StatPageInfo.SortNodeIP = template.HTML(TempStr)
...
func WebServer_Login_Check(w http.ResponseWriter, req *http.Request, Database *sql.DB) {
	...
	if req.Method == "POST" {
		...
		if Code == "200" {
			log.Println("[LOG] : Normal operation : 200")
		} else if Code == "000" {
			log.Println("[LOG] : Normal operation : 000")
		} else if Code == "700" {
			log.Println("[LOG] : Take place problem to Network")
		} else if Code == "650" {
			log.Println("[LOG] : ")
			WebServer_Redirect(w, req, "/login")
		} else if Code == "651" {
			log.Println("[LOG] : Take place problem to Network")
			WebServer_Redirect(w, req, "/login_svc")
		} else if Code == "652" {
			log.Println("[LOG] : End of service period")
			WebServer_Redirect(w, req, "/login_svc")
		} else if Code == "653" {
			log.Println("[LOG] :  product service restrictions (measured rate system)")
			WebServer_Redirect(w, req, "/login_svc")
		} else {
			log.Println("[LOG] : stop operation")
			WebServer_Redirect(w, req, "/login_svc")
		}
...
func SqliteDBInsertCodeValue(Database *sql.DB, tablename string, columnname string, Code string) {
	var DB_Flag int64
	var salt string
	var checksum string

	salt = "certification_code_value"
	hashing_algorithm := md5.New()
	HashingText := salt + ":" + Code
	hashing_algorithm.Write([]byte(HashingText))
	checksum = hex.EncodeToString(hashing_algorithm.Sum(nil))

	EncryptCode := AESEncryptEncodingValue(salt + ":" + Code + ":" + checksum)
	InsertDataStr := fmt.Sprintf("INSERT INTO %s (%s) VALUES ('%s')", tablename, columnname, EncryptCode)

	DB_Flag, _ = sqlitedb_lib.Insert_Data(Database, InsertDataStr)
	if DB_RET_FAIL == DB_Flag {
		log.Println("[ERR] Insert dabase err")
	}
}
...
func TrafficSqliteDBInit(DbName string) *sql.DB {
	...
	DB_Flag, _ = sqlitedb_lib.Create_Table(Database, "CREATE TABLE IF NOT EXISTS clienttrafficinfo (seq INTEGER PRIMARY KEY AUTOINCREMENT, mcseip TEXT,prevpeersumtraffic  BIGINT ,prevproxysumtraffic  BIGINT,proxycnt INT,clientcnt INT ,clienttrafficseqnum INT)")
	if DB_RET_FAIL == DB_Flag {
		log.Println("Create Table Fail!")
	}

	DB_Flag, _ = sqlitedb_lib.Create_Table(Database, "CREATE TABLE IF NOT EXISTS servertrafficinfo (seq  INTEGER PRIMARY KEY AUTOINCREMENT,mcseip TEXT,prevpeersumtraffic BIGINT,prevproxysumtraffic BIGINT,proxycnt INT,clientcnt INT,servercnt INT,servertrafficseqnum INT)")
	if DB_RET_FAIL == DB_Flag {
		log.Println("Create Table Fail!")
	}
...
func LocalWebServerSqliteDBInit(DbName string) *sql.DB {
	...
	query := "CREATE TABLE IF NOT EXISTS TrialAuthCodeTbl (" +
	...
	RowCount, _ = sqlitedb_lib.RowCount(db, "TrialAuthCodeTbl")
	if RowCount == 0 {
		SqliteDBInsertCodeValue(db, "TrialAuthCodeTbl", "TrialAuthCode", "000")
	}

	query = "CREATE TABLE IF NOT EXISTS CodeTbl (" +
		"Seq                 INTEGER NOT NULL PRIMARY KEY AUTOINCREMENT," +
		"Code            TEXT" +
		");"
	...
	RowCount, _ = sqlitedb_lib.RowCount(db, "CodeTbl")
	if RowCount == 0 {
		SqliteDBInsertCodeValue(db, "CodeTbl", "Code", "000")
	}

	query = "CREATE TABLE IF NOT EXISTS VersionModeTbl (" +
		"Seq                 INTEGER NOT NULL PRIMARY KEY AUTOINCREMENT," +
		"svc_type            TEXT," +
		"scenario_state      TEXT" +
		");"
...
func UpdateVersionCheck(svctype string, scenariostate string, database *sql.DB) {
	var svctypechecksum, scenariostatechecksum string
	var hashing_algorithm hash.Hash
	var HashingText string
	var err error
	var Encrypttextsvctype, Encrypttextscenariostate string
	var tx *sql.Tx
	var stmt *sql.Stmt

	var QueryStr string
	hashing_algorithm = md5.New()
	HashingText = "version_selected" + ":" + svctype
	hashing_algorithm.Write([]byte(HashingText))
	svctypechecksum = hex.EncodeToString(hashing_algorithm.Sum(nil))
	Encrypttextsvctype = AESEncryptEncodingValue("version_selected" + ":" + svctype + ":" + svctypechecksum)

	log.Println("svctypechecksum:", HashingText)
	hashing_algorithm = md5.New()
	HashingText = "scenariostate_selected" + ":" + scenariostate
	hashing_algorithm.Write([]byte(HashingText))
	scenariostatechecksum = hex.EncodeToString(hashing_algorithm.Sum(nil))
	Encrypttextscenariostate = AESEncryptEncodingValue("scenariostate_selected" + ":" + scenariostate + ":" + scenariostatechecksum)

	log.Println("svctypechecksum:", HashingText)
	tx, err = sqlitedb_lib.DB_Begin(database)
	if tx == nil {
		log.Println("Version UPdate  DB Begin err")

	}

	QueryStr = "update VersionModeTbl set svc_type=?, scenario_state =?"
	stmt, err = tx.Prepare(QueryStr)
	if err != nil {
		log.Println("Prepare Fail!:", err)
	}
	_, err = stmt.Exec(Encrypttextsvctype, Encrypttextscenariostate)
	if err != nil {
...
func InsertVersionCheck(svctype string, scenariostate string, database *sql.DB) {
	var svctypechecksum, scenariostatechecksum string
	var hashing_algorithm hash.Hash
	var HashingText string
	var err error
	...
	QueryStr = "Insert into VersionModeTbl  (svc_type, scenario_state) values(?,?)"
	stmt, err = tx.Prepare(QueryStr)
	if err != nil {
		log.Println("Prepare Fail!:", err)
	}
...
func SelectVersionCheck(database *sql.DB) (string, string, error) {
	var svctype, svctypesalt, svctypechecksum string
	var scenariostate, scenariostatesalt, scenariostatechecksum string
	var Decryptscenariostate, Decryptsvctype string
	var arrsvctype, arrscenariostate []string
	var hashing_algorithm hash.Hash
	var HashingText string
	var comparesvctypechecksum, comparescenariostatechecksum string
	var err error
	var QueryStr string
	var Rows *sql.Rows

	QueryStr = "select svc_type, scenario_state  from VersionModeTbl"
	Rows, err = sqlitedb_lib.Query_DB(database, QueryStr)
	if err != nil {
		log.Println("Query err", err)
	}
	...
	for Rows.Next() {
		err = Rows.Scan(&svctype, &scenariostate)
		if err != nil {
			log.Println("Scan error", err)
			Rows.Close()
			break
	...
	if svctype != "" {
		Decryptsvctype = AESDecryptDecodeValue(svctype)
		arrsvctype = strings.Split(Decryptsvctype, ":")
		if len(arrsvctype) != 3 {
			log.Println("Modulate svctype")
			return "", "", errors.New("Invalid svctype Info")
		}

		log.Println("Decryptsvctype:", Decryptsvctype)
		svctypesalt = arrsvctype[0]
		svctype = arrsvctype[1]
		svctypechecksum = arrsvctype[2]

		hashing_algorithm = md5.New()
		HashingText = svctypesalt + ":" + svctype
		hashing_algorithm.Write([]byte(HashingText))
		comparesvctypechecksum = hex.EncodeToString(hashing_algorithm.Sum(nil))
		if comparesvctypechecksum != svctypechecksum {
			log.Println("Modulate svctype")
			return "", "", errors.New("Invalid svctype checksum")
		}

		Decryptscenariostate = AESDecryptDecodeValue(scenariostate)
		arrscenariostate = strings.Split(Decryptscenariostate, ":")
		if len(arrscenariostate) != 3 {
			log.Println("Modulate scenariostate")
			return "", "", errors.New("Invalid scenariostate Info")
		}

		log.Println("Decryptscenariostate:", Decryptscenariostate)
		scenariostatesalt = arrscenariostate[0]
		scenariostate = arrscenariostate[1]
		scenariostatechecksum = arrscenariostate[2]

		hashing_algorithm = md5.New()
		HashingText = scenariostatesalt + ":" + scenariostate
		hashing_algorithm.Write([]byte(HashingText))
		comparescenariostatechecksum = hex.EncodeToString(hashing_algorithm.Sum(nil))
		if comparescenariostatechecksum != scenariostatechecksum {
			log.Println("Modulate scenariostate")
			return "", "", errors.New("Invalid scenariostate checksum")
		}

	}

	return svctype, scenariostate, nil
}
...
func RunLocalWebServer(bindPort string) {
	...
	WebServerMux.HandleFunc("/", func(w http.ResponseWriter, req *http.Request) {
		svctype, scenariostate, err := SelectVersionCheck(lwsDatabase)
		if err != nil {
			log.Println("select version error")
		}
		if svctype == "" && scenariostate == "" {
			WebServer_Redirect(w, req, "/versionselect/")
		} else if svctype == "trial" && scenariostate == "trial_certified" {
			WebServer_Redirect(w, req, "/login/")
		} else if svctype == "svc" && scenariostate == "trial_update" {
			//----------------update logic-----------
			WebServer_Redirect(w, req, "/trial_update/")
			//----------------update logic-----------
		} else {
			WebServer_Redirect(w, req, "/versionselect/")
		}
	...
	WebServerMux.HandleFunc("/versionselect/", func(w http.ResponseWriter, req *http.Request) {
		WebServer_Ver_Select(w, req, lwsDatabase)
	})

	WebServerMux.HandleFunc("/trial_auth/", func(w http.ResponseWriter, req *http.Request) {
		WebServer_Trial_Auth(w, req, lwsDatabase)
	})
	...
	WebServerMux.HandleFunc("/login/", func(w http.ResponseWriter, req *http.Request) {
		...
		if svctype == "trial" && scenariostate == "trial_certified" {
	...
	WebServerMux.HandleFunc("/save_trialid/", func(w http.ResponseWriter, req *http.Request) {
		WebServer_Save_TrialID(w, req, lwsDatabase)
	})

	//------------------------------------to do review ------------------------------------------
	WebServerMux.HandleFunc("/login_svc/", func(w http.ResponseWriter, req *http.Request) {
		WebServer_Login_SVC(w, req, lwsDatabase)
	})
	WebServerMux.HandleFunc("/logging_svc/", func(w http.ResponseWriter, req *http.Request) {
		WebServer_Logging_SVC(w, req, lwsDatabase, trafficDatabase)
	})

	WebServerMux.HandleFunc("/trial_update/", func(w http.ResponseWriter, req *http.Request) {
		svctype, scenariostate, err := SelectVersionCheck(lwsDatabase)
		if err != nil {
			log.Println("select version error")
		}
		if svctype == "svc" && scenariostate == "trial_update" {
			WebServer_Trial_Update(w, req, lwsDatabase, trafficDatabase)
		} else {
			WebServer_Redirect(w, req, "/versionselect/")
		}
	})
	...
	WebServerMux.HandleFunc("/tr_setting/", func(w http.ResponseWriter, req *http.Request) {
	...
	//---------------------------------{ 불필요함 지워도 됨 ------------------
	WebServerMux.HandleFunc("/auth_svc_checksum/v1.0/", func(w http.ResponseWriter, req *http.Request) {
		WebServer_Web_Auth_SVC_Checksum(w, req)
	})

	WebServerMux.HandleFunc("/auth_api_svc_setup_authtoken/v1.0/", func(w http.ResponseWriter, req *http.Request) {
		WebServer_Web_Auth_API_SVC_Setup_AuthToken(w, req)
	})
	WebServerMux.HandleFunc("/auth_api_encode_value/v1.0/", func(w http.ResponseWriter, req *http.Request) {
	...
	//----------------------------------------------}
	go HttpListen(0, bind, "", "", WebServerMux)
	go Traffic_Trans(lwsDatabase, trafficDatabase)
	go Status_File_Update()
...
func Status_File_Update() {
	var statusupdateccyc int
	var statusfd *os.File
	var status, statussalt, statusexpiretime, statuschecksum string
	var arrtstatus []string
	...
	for {
		select {
		case <-time.After(time.Second * time.Duration(statusupdateccyc)):
		}

		if _, err = os.Stat("./cfg/status.key"); os.IsNotExist(err) {
			continue
		} else {
			statusdata, err := ioutil.ReadFile("./cfg/status.key")
			if err != nil {
				log.Println(err)
			}

			Decrypttext = AESDecryptDecodeValue(string(statusdata))
			arrtstatus = strings.Split(Decrypttext, "|")
			if len(arrtstatus) != 4 {
				log.Println("Invlaid Status Key")
				continue
			}
			statussalt = arrtstatus[0]
			status = arrtstatus[1]
			statusexpiretime = arrtstatus[2]
			statuschecksum = arrtstatus[3]
			...
			if status == "start" {
				...
				log.Println("not update expire time")
			}
...
func Traffic_Trans(lwsDatabase *sql.DB, trafficDatabase *sql.DB) {

	var trialiddata []byte
	//var statusinfo []byte
	//var statusdata string
	var serverrowcnt int
	var clientrowcnt int
	var err error
	TrafficSendCycTime = 60
	for {
		select {
		case <-time.After(time.Minute * time.Duration(TrafficSendCycTime)):
		}

		if _, err := os.Stat("./cfg/status.key"); os.IsNotExist(err) {
			log.Println("[LOG] : not exist status.key")
			continue
		} else {
			log.Println("[LOG] : exist status.key")
		}
		...
		controlServerAddr := "mcstrial.uxcloud.net" + ":" + "9091"
		url := fmt.Sprintf("http://%s/auth_api/trial_traffic/v1.0/", controlServerAddr)

		if _, err := os.Stat("./cfg/trialid.key"); os.IsNotExist(err) {
			log.Println("not exist trialid.key")
			continue
		} else {
			log.Println("exist trialid.key")
			trialiddata, err = ioutil.ReadFile("./cfg/trialid.key")
			if err != nil {
				log.Println(err)
				continue
			}
		}
		trialidinfo := string(trialiddata)
		DecryptText := AESDecryptDecodeValue(trialidinfo)

		arrtrialid := strings.Split(DecryptText, ":")
		if len(arrtrialid) != 3 {
			log.Println("Modulate trialidinfo")
			continue
		}
		log.Println("DecryptText:", DecryptText)

		saltvalue := arrtrialid[0]
		trialid := arrtrialid[1]
		checksum := arrtrialid[2]

		hashing_algorithm := md5.New()
		HashingText := saltvalue + ":" + trialid
		hashing_algorithm.Write([]byte(HashingText))
		Hashingtrialidinfo := hex.EncodeToString(hashing_algorithm.Sum(nil))

		if Hashingtrialidinfo != checksum {
			log.Println("Modulate checksum ")
			continue
		}
		EncryptTrialid := AESEncryptEncodingValue(saltvalue + ":" + trialid)
		log.Println("trialid:", trialid)
		log.Println("saltvalue:", saltvalue)
		log.Println("EncryptTrialid:", EncryptTrialid)

		//----------------------------client traffic logic ---------------------------------
		err = Client_Traffic(trialid, saltvalue, EncryptTrialid, url, clientrowcnt, trafficDatabase)
		if err != nil {
			log.Println("[LOG] : client traffic error:", err)
			continue
		}
		err = Server_Traffic(trialid, saltvalue, EncryptTrialid, url, serverrowcnt, trafficDatabase)
		if err != nil {
			log.Println("[LOG] : server traffic error:", err)
			continue
		}
		//----------------------------server traffic logic ---------------------------------
	}
}
func Client_Traffic(trialid string, saltvalue string, EncryptTrialid string, url string, clientrowcnt int, trafficDatabase *sql.DB) error {

	var ClientTrafficInfo TrafficInformation
	var QueryStr string
	var Trafficseq, Curtrafficseq, Maxtrafficseq int
	var MCSEIP string
	var stmt *sql.Stmt
	var tx *sql.Tx
	var Prevpeersumtraffic, Prevproxysumtraffic, Curpeertraffic, Curproxytraffic, Peersumtraffic, Proxysumtraffic int64
	var Proxycnt, Clientcnt int
	var TraffRows *sql.Rows
	var IsClientOK bool
	var err error
	//----------------------------client traffic logic ---------------------------------

	IsClientOK = true
	QueryStr = "select COUNT(*) from clienttrafficinfo"
	TraffRows, err = sqlitedb_lib.Query_DB(trafficDatabase, QueryStr)
	if err != nil {
		log.Println("Query err", err)
		IsClientOK = false
	}
	...
	for TraffRows.Next() {
		err = TraffRows.Scan(&clientrowcnt)
		if err != nil {
			log.Println("Scan error", err)
			TraffRows.Close()
			IsClientOK = false
	...
	QueryStr = "select ifnull(max(id),0) from  Client_Statistics_Common"
	TraffRows, err = sqlitedb_lib.Query_DB(trafficDatabase, QueryStr)
	if err != nil {
		log.Println("Query err", err)
		IsClientOK = false
	...
	for TraffRows.Next() {
		err = TraffRows.Scan(&Maxtrafficseq)
		if err != nil {
			log.Println("Scan error", err)
			TraffRows.Close()
			IsClientOK = false
	...
	if Maxtrafficseq == 0 {
		log.Println("[LOG] : no exist Client Statistic Info")
		return nil
	}

	QueryStr = "select clienttrafficseqnum from clienttrafficinfo"
	TraffRows, err = sqlitedb_lib.Query_DB(trafficDatabase, QueryStr)
	if err != nil {
		log.Println("Query err", err)
		IsClientOK = false
	...
	if Maxtrafficseq > Trafficseq {

		QueryStr = "select prevpeersumtraffic,prevproxysumtraffic,clienttrafficseqnum from clienttrafficinfo"
		TraffRows, err = sqlitedb_lib.Query_DB(trafficDatabase, QueryStr)
		if err != nil {
			log.Println("Query err", err)
			IsClientOK = false
		...
		for TraffRows.Next() {
			err = TraffRows.Scan(&Prevpeersumtraffic, &Prevproxysumtraffic, &Trafficseq)
			if err != nil {
				log.Println("Scan error", err)
				TraffRows.Close()
				IsClientOK = false
		...
		ClientTrafficInfo.Prevpeersumtraffic = strconv.FormatInt(Prevpeersumtraffic, 10)
		ClientTrafficInfo.Prevproxysumtraffic = strconv.FormatInt(Prevproxysumtraffic, 10)

		QueryStr = "select  A.Node_IP_TEXT, Count(Distinct A.Client_IP_TEXT),Count(Distinct B.Proxy_IP_TEXT)\n" +
			"from Client_Statistics_Common A, Client_Statistics_Data B"
		...
		for TraffRows.Next() {
			err = TraffRows.Scan(&MCSEIP, &Clientcnt, &Proxycnt)
			if err != nil {
				log.Println("Scan error", err)
				TraffRows.Close()
				IsClientOK = false
		...
		TraffRows.Close()
		QueryStr = "select max(id),(select Sum(Inbound) from Client_Statistics_Data where id > %d ) total , (select Sum(Outbound) from Client_Statistics_Data where id > %d) total\n" +
			"from Client_Statistics_Data "

		QueryStr = fmt.Sprintf(QueryStr, Trafficseq, Trafficseq)
		TraffRows, err = sqlitedb_lib.Query_DB(trafficDatabase, QueryStr)
		if err != nil {
			log.Println("Query err", err)
			IsClientOK = false
		...
		for TraffRows.Next() {
			err = TraffRows.Scan(&Curtrafficseq, &Curpeertraffic, &Curproxytraffic)
			if err != nil {
				log.Println("Scan error", err)
				TraffRows.Close()
				IsClientOK = false
		...
		TraffRows.Close()
		ClientTrafficInfo.MCSEMode = "001"
		ClientTrafficInfo.MCSEIP = MCSEIP
		ClientTrafficInfo.Proxycnt = strconv.Itoa(Proxycnt)
		ClientTrafficInfo.Clientcnt = strconv.Itoa(Clientcnt)
		ClientTrafficInfo.Servercnt = strconv.Itoa(0)
		ClientTrafficInfo.Curproxytraffic = strconv.FormatInt(Curproxytraffic, 10)
		ClientTrafficInfo.Curpeertraffic = strconv.FormatInt(Curpeertraffic, 10)
		ClientTrafficInfo.Curpeersumtraffic = strconv.FormatInt(Prevpeersumtraffic+Curpeertraffic, 10)
		ClientTrafficInfo.Curproxysumtraffic = strconv.FormatInt(Prevproxysumtraffic+Curproxytraffic, 10)

		if IsClientOK == true {
			authkey, authtoken, err := ClientTrafficAuthRequest(url, EncryptTrialid, trialid, saltvalue)
			if err != nil {
				log.Println(err)
				TraffRows.Close()
				log.Println("ClientTrafficInfo ---->", ClientTrafficInfo)
				authcode := Check_Code_Checksum("Code", "CodeTbl")
				if authcode == "700" {
					log.Println("[ERR] : Take place Network problem")
					Status_File_Stop_Setting()
				} else if authkey == "652" {
					log.Println("[ERR] : Expired Trial Service")
					Status_File_Stop_Setting()
					return err
				} else if authkey == "653" {
					log.Println("[ERR] : Limited Trial Traffic")
					Status_File_Stop_Setting()
					return err
				} else if authkey == "654" {
					log.Println("[ERR] : Already Use Trial ID")
					Status_File_Stop_Setting()
					return err
				} else {
					log.Println("[ERR] Can't use Trial")
					Status_File_Stop_Setting()
					return err
				}
			} else {
				if err = ClientTrafficUploadRequest(url, trialid, saltvalue, authkey, authtoken, ClientTrafficInfo); err != nil {
					log.Println(err)
					TraffRows.Close()
					log.Println("ClientTrafficInfo ---->", ClientTrafficInfo)
					authcode := Check_Code_Checksum("Code", "CodeTbl")
					if authcode == "700" {
						...
						log.Println("[ERR] : Already User Trial ID")
						Status_File_Stop_Setting()
						return err
					} else {
						log.Println("[ERR] Can't use Trial")
					...
					Peersumtraffic = Prevpeersumtraffic + Curpeertraffic
					Proxysumtraffic = Prevproxysumtraffic + Curproxytraffic

					tx, err = sqlitedb_lib.DB_Begin(trafficDatabase)
					if tx == nil {
						log.Println("Traffic DB Begin err")
					}
					if clientrowcnt == 0 {
						QueryStr = "insert into clienttrafficinfo (mcseip, prevpeersumtraffic, prevproxysumtraffic, proxycnt, clientcnt, clienttrafficseqnum)\n" +
							"VALUES(?,?,?,?,?,?)"
					} else {
						QueryStr = "update clienttrafficinfo set mcseip=?, prevpeersumtraffic =?, prevproxysumtraffic = ? , proxycnt=? , clientcnt=?, clienttrafficseqnum=?"
					}
					stmt, err = tx.Prepare(QueryStr)
					if err != nil {
						log.Println("Prepare Fail!:", err)
					}
					_, err = stmt.Exec(MCSEIP, Peersumtraffic, Proxysumtraffic, Proxycnt, Clientcnt, Curtrafficseq)
					if err != nil {
						log.Println("Query:", err)
						sqlitedb_lib.DB_Rollback(tx)
					}
					...
					stmt.Close()
					err = Start_Status_Key()
					if err != nil {
						log.Println("[ERR] Status Key not Update : ", err)
					}
		...
		QueryStr = "select mcseip, prevpeersumtraffic,prevproxysumtraffic, proxycnt, clientcnt  from clienttrafficinfo"
		TraffRows, err = sqlitedb_lib.Query_DB(trafficDatabase, QueryStr)
		if err != nil {
			log.Println("Query err", err)
			IsClientOK = false
		...
		for TraffRows.Next() {
			err = TraffRows.Scan(&MCSEIP, &Prevpeersumtraffic, &Prevproxysumtraffic, &Proxycnt, &Clientcnt)
			if err != nil {
				log.Println("Scan error", err)
				TraffRows.Close()
				IsClientOK = false
		...
		if clientrowcnt == 1 {
			...
			ClientTrafficInfo.Prevpeersumtraffic = strconv.FormatInt(Prevpeersumtraffic, 10)
			ClientTrafficInfo.Prevproxysumtraffic = strconv.FormatInt(Prevproxysumtraffic, 10)
			ClientTrafficInfo.Curpeertraffic = strconv.Itoa(0)
			ClientTrafficInfo.Curproxytraffic = strconv.Itoa(0)
			ClientTrafficInfo.Curpeersumtraffic = strconv.FormatInt(Prevpeersumtraffic, 10)
			ClientTrafficInfo.Curproxysumtraffic = strconv.FormatInt(Prevproxysumtraffic, 10)
		} else {
			ClientTrafficInfo.MCSEMode = "001"
			ClientTrafficInfo.MCSEIP = ""
			ClientTrafficInfo.Proxycnt = strconv.Itoa(0)
			ClientTrafficInfo.Clientcnt = strconv.Itoa(0)
			ClientTrafficInfo.Servercnt = strconv.Itoa(0)
			ClientTrafficInfo.Prevpeersumtraffic = strconv.Itoa(0)
			ClientTrafficInfo.Prevproxysumtraffic = strconv.Itoa(0)
			ClientTrafficInfo.Curpeertraffic = strconv.Itoa(0)
			ClientTrafficInfo.Curproxytraffic = strconv.Itoa(0)
			ClientTrafficInfo.Curpeersumtraffic = strconv.Itoa(0)
			ClientTrafficInfo.Curproxysumtraffic = strconv.Itoa(0)
		}
...
func Start_Status_Key() error {

	statusfd, err := os.Create("./cfg/status.key")
	if err != nil {
		log.Println("[ERR] create status file err :", err)
		return err
	}
...
func Server_Traffic(trialid string, saltvalue string, EncryptTrialid string, url string, serverrowcnt int, trafficDatabase *sql.DB) error {

	var ServerTrafficInfo TrafficInformation
	var QueryStr string
	var Trafficseq, Curtrafficseq, Maxtrafficseq int
	var MCSEIP string
	var stmt *sql.Stmt
	var tx *sql.Tx
	var Prevpeersumtraffic, Prevproxysumtraffic, Curpeertraffic, Curproxytraffic, Peersumtraffic, Proxysumtraffic int64
	var Proxycnt, Clientcnt, Servercnt int
	var TraffRows *sql.Rows
	var IsServerOK bool
	var err error
	...
	QueryStr = "select COUNT(*) from servertrafficinfo"
	TraffRows, err = sqlitedb_lib.Query_DB(trafficDatabase, QueryStr)
	if err != nil {
		log.Println("Query err", err)
		IsServerOK = false
	...
	for TraffRows.Next() {
		err = TraffRows.Scan(&serverrowcnt)
		if err != nil {
			log.Println("Scan error", err)
			TraffRows.Close()
			IsServerOK = false
	...
	TraffRows.Close()
	QueryStr = "select  ifnull(max(id),0) from  Server_Statistics_Common"
	TraffRows, err = sqlitedb_lib.Query_DB(trafficDatabase, QueryStr)
	if err != nil {
		log.Println("Query err", err)
		IsServerOK = false
	...
	TraffRows.Close()
	if Maxtrafficseq == 0 {
		log.Println("[LOG] : no exist Server Statistic Info")
		return nil
	}

	QueryStr = "select servertrafficseqnum from servertrafficinfo"
	TraffRows, err = sqlitedb_lib.Query_DB(trafficDatabase, QueryStr)
	if err != nil {
		log.Println("Query err", err)
		IsServerOK = false
	...
	TraffRows.Close()
	Maxtrafficseq = Maxtrafficseq
	if Maxtrafficseq > Trafficseq {

		QueryStr = "select prevpeersumtraffic,prevproxysumtraffic from servertrafficinfo"
		TraffRows, err = sqlitedb_lib.Query_DB(trafficDatabase, QueryStr)
		if err != nil {
			log.Println("Query err", err)
			IsServerOK = false
		...
		for TraffRows.Next() {
			err = TraffRows.Scan(&Prevpeersumtraffic, &Prevproxysumtraffic)
			if err != nil {
				log.Println("Scan error", err)
				TraffRows.Close()
				IsServerOK = false
		...
		TraffRows.Close()
		ServerTrafficInfo.Prevpeersumtraffic = strconv.FormatInt(Prevpeersumtraffic, 10)
		ServerTrafficInfo.Prevproxysumtraffic = strconv.FormatInt(Prevproxysumtraffic, 10)

		QueryStr = "select A.Node_IP_TEXT,Count(Distinct A.Proxy_IP_TEXT),Count(Distinct A.Server_IP_TEXT),Count(Distinct B.Client_IP_TEXT)\n" +
			"from Server_Statistics_Common A,Server_Statistics_Data B"
		...
		for TraffRows.Next() {
			err = TraffRows.Scan(&MCSEIP, &Proxycnt, &Servercnt, &Clientcnt)
			if err != nil {
				log.Println("Scan error", err)
				TraffRows.Close()
				IsServerOK = false
		...
		TraffRows.Close()
		QueryStr = "select max(id),(select Sum(Inbound) from Server_Statistics_Data where id > %d ) total , (select Sum(Outbound) from Server_Statistics_Data where id > %d) total\n" +
			"from Server_Statistics_Data"
		...
		TraffRows.Close()
		ServerTrafficInfo.MCSEMode = "002"
		ServerTrafficInfo.MCSEIP = MCSEIP
		ServerTrafficInfo.Proxycnt = strconv.Itoa(Proxycnt)
		ServerTrafficInfo.Clientcnt = strconv.Itoa(Clientcnt)
		ServerTrafficInfo.Servercnt = strconv.Itoa(Servercnt)
		ServerTrafficInfo.Curpeertraffic = strconv.FormatInt(Curpeertraffic, 10)
		ServerTrafficInfo.Curproxytraffic = strconv.FormatInt(Curproxytraffic, 10)
		ServerTrafficInfo.Curpeersumtraffic = strconv.FormatInt(Prevpeersumtraffic+Curpeertraffic, 10)
		ServerTrafficInfo.Curproxysumtraffic = strconv.FormatInt(Prevproxysumtraffic+Curproxytraffic, 10)

		if IsServerOK == true {
			authkey, authtoken, err := ServerTrafficAuthRequest(url, EncryptTrialid, trialid, saltvalue)
			if err != nil {
				log.Println(err)
				TraffRows.Close()
				log.Println("ServerTrafficInfo ----->", ServerTrafficInfo)
				authcode := Check_Code_Checksum("Code", "CodeTbl")
				if authcode == "700" {
					log.Println("[ERR] : Take place Network problem")
					Status_File_Stop_Setting()
				...
				if err = ServerTrafficUploadRequest(url, trialid, saltvalue, authkey, authtoken, ServerTrafficInfo); err != nil {
					log.Println(err)
					TraffRows.Close()
					...
					if serverrowcnt == 0 {
						QueryStr = "insert into servertrafficinfo (mcseip,prevpeersumtraffic, prevproxysumtraffic, proxycnt, servercnt,\n" +
							"clientcnt,servertrafficseqnum)\n" +
							"VALUES(?,?,?,?,?,?,?)"
					} else {
						QueryStr = "update servertrafficinfo set mcseip=?, prevpeersumtraffic =?, prevproxysumtraffic = ? , proxycnt=? , servercnt=? ,\n" +
							"clientcnt=?,servertrafficseqnum=?"
					}
					...
					_, err = stmt.Exec(MCSEIP, Peersumtraffic, Proxysumtraffic, Proxycnt, Servercnt, Clientcnt, Curtrafficseq)
					if err != nil {
						stmt.Close()
						log.Println("Query:", err)
						sqlitedb_lib.DB_Rollback(tx)
		...
		QueryStr = "select mcseip, prevpeersumtraffic,prevproxysumtraffic, proxycnt, clientcnt,servercnt  from servertrafficinfo"
		TraffRows, err = sqlitedb_lib.Query_DB(trafficDatabase, QueryStr)
		if err != nil {
			log.Println("Query err", err)
			IsServerOK = false
		...
		for TraffRows.Next() {
			err = TraffRows.Scan(&MCSEIP, &Prevpeersumtraffic, &Prevproxysumtraffic, &Proxycnt, &Clientcnt, &Servercnt)
			if err != nil {
				log.Println("Scan error", err)
				TraffRows.Close()
				IsServerOK = false
		...
		if serverrowcnt == 1 {
			...
			ServerTrafficInfo.Prevpeersumtraffic = strconv.FormatInt(Prevpeersumtraffic, 10)
			ServerTrafficInfo.Prevproxysumtraffic = strconv.FormatInt(Prevproxysumtraffic, 10)
			ServerTrafficInfo.Curpeertraffic = strconv.Itoa(0)
			ServerTrafficInfo.Curproxytraffic = strconv.Itoa(0)
			ServerTrafficInfo.Curpeersumtraffic = strconv.FormatInt(Prevpeersumtraffic, 10)
			ServerTrafficInfo.Curproxysumtraffic = strconv.FormatInt(Prevproxysumtraffic, 10)
		} else {
			ServerTrafficInfo.MCSEMode = "002"
			ServerTrafficInfo.MCSEIP = ""
			ServerTrafficInfo.Proxycnt = strconv.Itoa(0)
			ServerTrafficInfo.Clientcnt = strconv.Itoa(0)
			ServerTrafficInfo.Servercnt = strconv.Itoa(0)
			ServerTrafficInfo.Prevpeersumtraffic = strconv.Itoa(0)
			ServerTrafficInfo.Prevproxysumtraffic = strconv.Itoa(0)
			ServerTrafficInfo.Curpeertraffic = strconv.Itoa(0)
			ServerTrafficInfo.Curproxytraffic = strconv.Itoa(0)
			ServerTrafficInfo.Curpeersumtraffic = strconv.Itoa(0)
			ServerTrafficInfo.Curproxysumtraffic = strconv.Itoa(0)
		}
...
func Status_File_Stop_Setting() {
	...
	_, err = statusfd.Write([]byte(AESEncryptEncodingValue(statussalt + "|" + status + "|" + statusexpiretime + "|" + statuschecksum)))
	//_, err = statusfd.Write([]byte(statussalt + "|" + status + "|" + statusexpiretime + "|" + statuschecksum))
	if err != nil {
		log.Println(" Write err:", err)
	}
	defer statusfd.Close()
...
func ClientTrafficAuthRequest(url string, EncryptTrialid string, trialid string, saltvalue string) (string, string, error) {
	authReqProto := jsonInputWebAPIAuthTraffLocalPack{}
	authReqProto.Version = "1.0"
	authReqProto.Method = "auth"
	authReqProto.SessionType = "traffic"
	authReqProto.Seperator = "client"
	authReqProto.MessageType = "request"

	TrafficTranMsgSeq++
	if TrafficTranMsgSeq == 100000 {
		TrafficTranMsgSeq = 1
	}

	authReqProto.MsgSeq = strconv.Itoa(TrafficTranMsgSeq)
	authReqProto.Trialid = EncryptTrialid
	...
	for {
		...
		if err != nil {
			...
			log.Println("[ERR] : ", err)
			if retrynum == 3 {
				SqlitedbUpdateCode("CodeTbl", "Code", "700")
				return "", "", err
			}
		} else {
			if resp.StatusCode != 200 {
				...
				if retrynum == 3 {
					SqlitedbUpdateCode("CodeTbl", "Code", "700")
					return "", "", errors.New(fmt.Sprintf("Response Auth traffic Http error: %d", resp.Status))
				}
			} else {
				log.Println("[LOG] : Success client do")
				break
	...
	bodyBytes, _ := ioutil.ReadAll(resp.Body)
	authResProto := jsonOutputWebAPIAuthTraffLocalPack{}
	if err := json.Unmarshal(bodyBytes, &authResProto); err != nil {
		return "", "", errors.New(err.Error() + "\nurl: " + url)
	}

	log.Println("authResProto------>:", authResProto)

	if authResProto.Code != "200" {
		log.Println("authResProto.Code:", authResProto.Code)
		SqlitedbUpdateCode("CodeTbl", "Code", authResProto.Code)
		return "", "", errors.New("invailed auth protocol Code")
	}
	if authResProto.Version != "1.0" {
		return "", "", errors.New("invailed auth protocol Version")
	}
	...
	if authResProto.MsgType != "response" {
		return "", "", errors.New("invailed auth protocol MsgSeq " + "authReqProto.MsgSeq:" + authReqProto.MsgSeq + " authReqProto.MsgSeq:" + authReqProto.MsgSeq)
	}
	if authResProto.MsgSeq != authReqProto.MsgSeq {
		log.Println("authResProto.MsgSeq:", authResProto.MsgSeq)
		return "", "", errors.New("invailed auth protocol MsgSeq")
	}
	if authResProto.SessionType != authReqProto.SessionType {
		return "", "", errors.New("invailed auth protocol SessionType")
	}
	if authResProto.Seperator != authReqProto.Seperator {
		log.Println("authReqProto.Seperator:", authReqProto.Seperator)
		return "", "", errors.New("invailed auth protocol SessionType")
	}
	if authResProto.AuthKey == "" {
		return "", "", errors.New("invailed auth protocol AuthKey")
	...
	authtoken := WebServer_Auth_API_Hashing_Traffic(trialid, saltvalue, authResProto.Method, authResProto.AuthKey)
	log.Println("AuthKey", authResProto.AuthKey)
	log.Println("authtoken", authtoken)
	return authResProto.AuthKey, authtoken, nil
}
func ClientTrafficUploadRequest(url string, trialid string, saltvalue string, authkey string, authtoken string, TrafficInfo TrafficInformation) error {
	var DecryptTafficChecksum string
	...
	for {
		...
		if err != nil {
			...
			if resp.StatusCode != 200 {
				...
				if retrynum == 3 {
					SqlitedbUpdateCode("CodeTbl", "Code", "700")
					return errors.New(fmt.Sprintf("Response Traffic Http error: %d", resp.Status))
				}
			} else {
				log.Println("[LOG] : Success client do")
				break
	...
	bodyBytes, _ := ioutil.ReadAll(resp.Body)
	log.Println("[LOG] Response : ", string(bodyBytes))
	...
	if authResProto.Code != "200" {
		SqlitedbUpdateCode("CodeTbl", "Code", authResProto.Code)
		return errors.New("invailed auth protocol Code " + " authReqProto.Code:" + authResProto.Code)
	}
	if authResProto.Version != "1.0" {
		return errors.New("invailed auth protocol Version")
	}
	...
	if authResProto.Seperator != authReqProto.Seperator {
		return errors.New("invailed auth protocol Seperator")
	}
	if authResProto.MsgSeq != authReqProto.MsgSeq {
		return errors.New("invailed auth protocol MsgSeq " + "authReqProto.MsgSeq:" + authReqProto.MsgSeq + " authReqProto.MsgSeq:" + authReqProto.MsgSeq)
	}

	if authResProto.SendCycTime == "" {
		TrafficSendCycTime = 60
	} else {
		TrafficSendCycTime, err = strconv.Atoi(authResProto.SendCycTime)
		if err != nil {
			log.Println("authResProto.SendCycTime:", authResProto.SendCycTime)
			return err
		}
	}

	DecryptTafficChecksum = AESDecryptDecodeValue(authResProto.Param)
	arrchecksum := strings.Split(DecryptTafficChecksum, ":")
	if len(arrchecksum) != 3 {
		return errors.New("Modulate TrafficChecksum")
	}
	Resauthkey := arrchecksum[0]
	Restrialid := arrchecksum[1]
	Reschecksum := arrchecksum[2]

	hashing_algorithm := md5.New()
	HashingText := Resauthkey + ":" + Restrialid
	hashing_algorithm.Write([]byte(HashingText))
	comparechecksum := hex.EncodeToString(hashing_algorithm.Sum(nil))

	if comparechecksum != Reschecksum {
		return errors.New("Traffic invalid checksum")
	}

	return nil
}

func ServerTrafficAuthRequest(url string, EncryptTrialid string, trialid string, saltvalue string) (string, string, error) {
	authReqProto := jsonInputWebAPIAuthTraffLocalPack{}
	authReqProto.Version = "1.0"
	authReqProto.Method = "auth"
	authReqProto.SessionType = "traffic"
	authReqProto.Seperator = "server"
	authReqProto.MessageType = "request"
...
func ServerTrafficUploadRequest(url string, trialid string, saltvalue string, authkey string, authtoken string, TrafficInfo TrafficInformation) error {
	var DecryptTafficChecksum string
	...
	if resp.StatusCode != 200 {
		SqlitedbUpdateCode("CodeTbl", "Code", "700")
		return errors.New(fmt.Sprint("Response traffic Http error: %d", resp.Status))
	}

	bodyBytes, _ := ioutil.ReadAll(resp.Body)
	log.Printf("[LOG] TrafficuploadRequest(): Response json=%s\n", string(bodyBytes))
	...
	if authResProto.MsgSeq != authReqProto.MsgSeq {
		return errors.New("invailed auth protocol MsgSeq " + "authReqProto.MsgSeq:" + authReqProto.MsgSeq)
	}
	...
	if authResProto.SendCycTime == "" {
		...
		if err != nil {
			log.Println("authResProto.SendCycTime:", authResProto.SendCycTime)
			log.Println("[ERR] : TrafficSendCycTime Error")
		}
	}
...
func SqlitedbUpdateCode(tablename string, columnname string, Code string) {
	var DB_Flag int64
	var salt string
	var checksum string

	lwsDatabase := LocalWebServerSqliteDBInit(LocalWebServerDB)
	defer lwsDatabase.Close()
	...
	EncryptCode := AESEncryptEncodingValue(salt + ":" + Code + ":" + checksum)
	UpdateDataStr := fmt.Sprintf("Update %s Set %s = '%s' ", tablename, columnname, EncryptCode)

	DB_Flag, _ = sqlitedb_lib.Update_Data(lwsDatabase, UpdateDataStr)
	if DB_RET_FAIL == DB_Flag {
		log.Println("[ERR] Update CodeTbl err")
	}
}
func Check_Code_Checksum(column string, tablename string) string {

	var EncryptCode string
	var DecryptCode string
	var arrcodeinfo []string
	lwsDatabase := LocalWebServerSqliteDBInit(LocalWebServerDB)
	defer lwsDatabase.Close()

	QueryStr := "select %s from %s"
	QueryStr = fmt.Sprintf(QueryStr, column, tablename)
	Rows, err := sqlitedb_lib.Query_DB(lwsDatabase, QueryStr)
	if err != nil {
		log.Println("[ERR] Query err", err)
		return ""
	}
	...
	DecryptCode = AESDecryptDecodeValue(EncryptCode)

	arrcodeinfo = strings.Split(DecryptCode, ":")
	if len(arrcodeinfo) != 3 {
		log.Println("Invlaid Code Value")
		return ""
	}

	salt := arrcodeinfo[0]
	code := arrcodeinfo[1]
	checksum := arrcodeinfo[2]
	...
	if comparechecksum != checksum {
		log.Println("[LOG] Code checksum err")
		return ""
	}
...
func ShowHelpCommand() {
	...
	log.Println(" -l    Run as GOWEB")
	log.Println("")
	log.Println(" Available Options")
	log.Println(" -d	Run as Daemon")
	log.Println("")
	log.Println(" -v	Check GOWEB Version")
	log.Println("")
}
...
func Node_Cfg_Detail(w http.ResponseWriter, req *http.Request, Database *sql.DB) {
	...
	for i := range settingData.SiteList {
		...
		if Node_Mode == Node_MODE_NONE {
			...
			OptionStr = fmt.Sprintf("<option value=\"%d\">선택해주세요</option><option value=\"%d\">Node Client</option><option selected=\"selected\" value=\"%d\">SML Server</option>", Node_MODE_NONE, Node_MODE_CLIENT, Node_MODE_SERVER)
		}
...
func ProvisioningDownloadRequest(db *sql.DB, url string, userstlbData *UsersTlbData, authData *AuthData, authkey string, authtoken string, syncSeqNo int64) error {
	...
	if resp.StatusCode != 200 {
		return errors.New(fmt.Sprintf("Response Provisioning Http error: %d", resp.Status))
	}
	...
	log.Println("proviRes.Body.Data:", proviRes.Body.Data)
...
//----------------------------------------------------------
//----------------------------------------------------------
type jsonInputWebAPIAuthTraffLocalPack struct {
	Version     string      `json:"version"`
	Method      string      `json:"method"`
	SessionType string      `json:"sessiontype"`
	Seperator   string      `json:"seperator"`
...
type jsonOutputWebAPIAuthTraffLocalPack struct {
	Version     string `json:"version"`
	Method      string `json:"method"`
	SessionType string `json:"sessiontype"`
	Seperator   string `json:"seperator"`
...
//----------------------------------------------------------
func WebServer_Auth_API_Hashing_Traffic(trialid string, saltvalue string, Method string, GenerateAuthKey string) string {
	...
	hashing_algorithm = md5.New()
	HashingText = Method + ":" + "/auth_api/trial_traffic/v1.0/"
	hashing_algorithm.Write([]byte(HashingText))
	HA2 = hex.EncodeToString(hashing_algorithm.Sum(nil))
	EventValue += "[" + HashingText + " >> HA2:" + HA2 + "]"
...
func main() {
	...
	//--------------{ dukryung89@innogs.com	(date:2020.07.15) - desc: excute (trial -> service)  daemon restart failed (#issue number : 13347)
	if DaemonFlag == 1 {
		context := &daemon.Context{
			PidFileName: "Stat_Web_linux.pid",
			PidFilePerm: 0644,
			LogFileName: ProcessLogFileName,
			LogFilePerm: 0640,
			WorkDir:     "./",
			Umask:       027,
			Args:        []string{"./Stat_Web_linux", WebServerFlag, "-d"},
		}
