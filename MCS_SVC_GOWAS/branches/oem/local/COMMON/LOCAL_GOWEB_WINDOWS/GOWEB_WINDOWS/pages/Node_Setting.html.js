...
//		$Bridge_Buf_Size.val(yn == "Enable" ? "2097152" : "");
  ...
  function AuthKeyQuery (url, version, method, sessiontype, msgtype, userkey, nodeid, mactotal){
    var AuthKeyValue = ""
  ...
  function ConfigDataUpload (url, version, method, sessiontype, msgtype, userkey, nodeid, mactotal, authkey, authtoken, cseq, nseq, data) {
    var return_value = "";
  ...
  //-- Provisioning ----------------------------------------------------------------------}
    ...
    if ($.trim($Statistic_Send_Control_Server.val()) == "Enable" 
      ...
      if (UserKeyEncodeValue == "" || NodeIDEncodeValue == "") {
        alert ("UserKey 또는 NodeID를 Encryption 하는데 오류가 발생하였습니다.");
        return;
      }

      AuthKeyReturnValue = AuthKeyQuery(RemoteURL, Version, Method, SessionType, MessageType, UserKeyEncodeValue, NodeIDEncodeValue, MacTotalvalue);
      ...
      AuthTokenReturnValue = AuthTokenQuery(Method, SessionType, UserKeyInputValue, AuthKeyReturnValue);
      ...
      var ConfigResponseData = ConfigDataUpload (RemoteURL, Version, Method, SessionType, MessageType, UserKeyEncodeValue, NodeIDEncodeValue, MacTotalvalue, AuthKeyReturnValue, AuthTokenReturnValue, CurrentSeqValue, NextSeqValue, params);
      if (ConfigResponseData == "") {
        alert ("Provisioning Control Server로 Upload 하는데 실패하였습니다.");
        return;
      } 
      else if (ConfigResponseData == "200") {
        alert("Provisioning Control Server로 해당 정보를 Uploading 하였습니다.");
      } else if (ConfigResponseData == "650") {
        alert("Provisioning Control Server에서 다운받을 정보가 있습니다.\n10초 후에 화면을 갱신 후 저장기능을 사용하시기 바랍니다.");
        return;
      }
    }
